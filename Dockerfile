...
FROM swaggerapi/swagger-ui:v5.32.14 AS swagger-ui-source

# Start with OpenVINO runtime which has verified Intel NPU/GPU drivers.
# Held at 2026.2.1 (not the latest 2026.3.0): 2026.3.0's GPU plugin fails real
# UVR inference on Intel iGPU with native OpenCL errors, confirmed by A/B
# testing both versions against the same hardware — see pyproject.toml's
# openvino pin for details.
...
RUN find /usr/local/lib/python3.*/ /app/libs/whisperx/ -name "*.so*" -exec patchelf --clear-execstack {} \;
