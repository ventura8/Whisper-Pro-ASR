...
# Shared runtime tools only. build-essential and python3-dev are deliberately NOT here:
# they are build-time wheel toolchain, and installing them in this stage put their bytes in
# a layer that every target inherits. strip_build_artifacts.sh purges them later, which
# satisfies verify_no_build_artifacts.sh -- the files are gone from the final filesystem --
# but a purge in a descendant layer only writes a whiteout, so the ~250MB still shipped in
# this one. They now install and purge inside the single RUN of each deps stage below.
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,target=/var/lib/apt,sharing=locked \
  apt-get update && \
  apt-get install -y --no-install-recommends \
...
# /app/model_cache is created here, not only by compose's init-permissions service. The
# plain `docker run` invocations in docs/SETUP.md have no init container, and HF_HOME points
# inside this directory -- so without it provisioning tried to create a directory under
# root-owned /app as the `nobody` runtime user and failed on first start.
RUN mkdir -p /app/data && chmod 777 /app/data && \
  mkdir -p /app/model_cache && chmod 777 /app/model_cache && \
...
USER root
# The wheel toolchain is installed and purged within THIS single RUN (the purge is inside
# strip_build_artifacts.sh at the end), so the package bytes never reach a committed layer.
RUN --mount=type=cache,target=/root/.cache \
  ...
  # 2, not 4. BuildKit runs the independent stages of a target concurrently, so on the
  # heaviest one (amd-rocm-torch: the ROCm stack plus a forced torch reinstall) this resolve
  # overlaps ort-builder's four ONNX Runtime wheels and the apt fetches. Cold-built on
  # 2026-09-08 that combination exhausted the link and poetry died with ReadTimeoutError on
  # five consecutive attempts, a different package each time and never a resolution error --
  # while the same stage built alone passed every time. Halving poetry's own connection count
  # is the part of that contention this file controls.
...
USER root
# Same as deps-cuda: toolchain installed and purged inside one RUN, never a shipped layer.
RUN --mount=type=cache,target=/root/.cache \
  --mount=type=cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,target=/var/lib/apt,sharing=locked \
  apt-get update && \
  ...
  # Versions come from the installed distributions, not from parsing pyproject.toml. That
  # grep returned the raw declaration, so it only ever worked because these three happen to
  # be pinned to bare versions -- the day one becomes "^2.13.0" or a table, pip would be
  # handed `torch==^2.13.0`. importlib.metadata reports what poetry.lock actually resolved
  # and poetry just installed, which is precisely what must be replaced.
  torch_v="$(python3 -c 'import importlib.metadata as m; print(m.version("torch"))')" && \
  audio_v="$(python3 -c 'import importlib.metadata as m; print(m.version("torchaudio"))')" && \
  vision_v="$(python3 -c 'import importlib.metadata as m; print(m.version("torchvision"))')" && \
  # --no-deps alongside --force-reinstall: without it pip reinstalls torch's whole
  # dependency tree from the CPU index too, overriding versions poetry.lock resolved and
  # pinned. Only the three wheels named here are meant to be swapped.
  PYTHONDONTWRITEBYTECODE=1 python3 -m pip install --no-cache-dir --force-reinstall --no-deps \
...
# The `|| true` is braced around the find alone. Unbraced it applied to the whole && chain,
# because `||` binds looser than `&&`: a failed onnxruntime install then produced a
# *successful* build with an empty /out, which the cpu, intel, nvidia and nvidia-intel
# targets copy from. A missing provider surfaces at runtime as a silent CPU fallback.
...
# Appended, not prepended, for the same reason as sys-full below: the base image's path
# puts the system OpenCL ICD loader first, and putting /opt/rocm/lib in front overrides it
# with ROCm's own. sys-amd inherits deps-cpu and has no Intel plugin of its own, so the
# ordering matters less here than in sys-full -- it is kept identical so the two stages
# cannot drift into disagreeing about which loader wins.
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
  ...
  /usr/local/build/strip_build_artifacts.sh
ENV PATH=/opt/rocm/bin:${PATH}
# Appended, not prepended. sys-full inherits sys-nvidia-intel, whose base path puts the
# system OpenCL ICD loader first so the OpenVINO GPU plugin can enumerate
# /etc/OpenCL/vendors/intel.icd. Putting /opt/rocm/lib in front overrode that loader
# with ROCm's own, and the Intel GPU disappeared from a target that exists to have both.
ENV LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/opt/rocm/lib
ENV ROCBLAS_TENSILE_LIBPATH=/usr/lib/x86_64-linux-gnu/rocblas/current/library
ENV HSA_ENABLE_DXG_DETECTION=1
USER nobody
...
# NVIDIA-only WhisperX option; `full` ships WhisperX as well, alongside every other vendor's
# runtime. WhisperX runs on CUDA in both because they reuse this image's torch rather than
# the CPU build its lock pins.
...
# Stage 10: Every vendor in one image, plus WhisperX. Supported, but the wrong default for a
# host whose hardware IS known: every capability it offers is available from a smaller,
# purpose-built target above, and it is by far the largest. Its reason to exist is a host
# whose hardware is not known ahead of time, or one image serving a mixed fleet -- which is
# also why it carries the unsuffixed `latest` tag in .github/workflows/ci.yml. Matches the
# "Which Image Should I Pick?" guidance in README.md; keep the two in step.
