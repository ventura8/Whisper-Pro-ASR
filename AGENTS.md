...
- **Hardware Validation After Every Review Wave (Mandatory)**: A CodeRabbit (or any other) review wave is NOT finished when the mocked suite is green. The whole suite mocks the ASR engine, so a broken accelerator path passes every one of its tests. Before reporting a wave complete you MUST run real-engine validation on real hardware for the paths the wave touched, via `scripts/remote_validate.sh <user>@<host> --full` (or `scripts/validation_matrix.sh`), and report the outcome. Map the change to the host that can actually exercise it: engine/device/pool resolution in `modules/core/config*.py` needs the host whose accelerator the change concerns (the Intel NPU box for NPU logic, an NVIDIA box for CUDA, a hybrid box for preprocessor/isolation routing); preprocessing and UVR changes need vocal separation actually enabled; manifest or scoring changes need the real-audio matrix. If a host is unreachable, say which change is therefore unvalidated -- never silently substitute the mocked run. This rule exists because it was skipped: the 2026-09-06 wave shipped a config change that removed a rejected NPU from the scheduler pool, passed 1826 mocked tests, and then failed 8 of 9 accuracy tests on the NUC because a CPU-typed unit made `engine_factory` hand the OpenVINO IR to CTranslate2.
...
- **Testing a module's internals by name is expected, not a smell.** Several modules present a
  thin public wrapper over rules that matter individually; reaching a private name directly is
  how those rules stay pinned. Tests that do this no longer each restate the reasoning.
