...
def run_language_detection_core(model, audio_input, skip_vad=False):
    ...
    if _should_detect_over_the_pipe(model, audio_input):
        # The same fallback the in-process path gets. Without it a detect_language failure in
        # the worker propagated out of run_batch_language_detection_direct's per-window loop
        # and lost every remaining window, where an in-process engine would have degraded to
