...
#: Successfully-read architectures, keyed by device id. Only positive answers are stored:
#: see :func:`intel_gpu_arch_for`.
_ARCH_CACHE: dict[str, tuple[int, int]] = {}
...
def intel_gpu_arch_for(device_id: str) -> tuple[int, int] | None:
    ...
    Cached per device id. A GPU's generation cannot change while the process runs, and the
    answer costs an ``ov.Core()`` construction -- which loads every installed plugin -- plus
    a property read. create_manager consults this on every preprocessor build, so an
    uncached probe re-enumerated the whole OpenVINO device tree per manager.

    Only a successful read is cached. None means "could not tell", which is a property of
    the attempt rather than of the device -- a plugin still loading, or a driver momentarily
    busy. An lru_cache stored that transient answer permanently, so one unlucky probe during
    startup pinned an Arc GPU to the slow in-process path for the life of the process.
    """
    cached = _ARCH_CACHE.get(str(device_id))
    if cached is not None:
        return cached
    try:
        parsed = _read_intel_gpu_arch(device_id)
...
def _read_intel_gpu_arch(device_id: str) -> tuple[int, int] | None:
    """Ask OpenVINO directly, with no caching. See :func:`intel_gpu_arch_for` for why."""
    ...
    if _INTEL_VENDOR_ID not in architecture:
        logger.info(
            "[Preprocess] OpenVINO's %s is not confirmed Intel silicon (%s); its architecture "
            "says nothing about the Intel GPU, so vocal isolation stays in-process.",
            target,
            architecture,
        )
        return None
