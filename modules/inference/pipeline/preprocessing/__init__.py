...
#: Rebound by _lazy_import_separator through ``sys.modules[__name__]`` rather than a
#: `global` statement -- the same module-attribute pattern history_manager uses for its
#: caches. That keeps the name UPPER-case-exempt without an invalid-name suppression and
#: drops the global-statement/PLW0603 pair, all of which this repository bans inline.
Separator = None
...
def _lazy_import_separator():
    ...
    if module.Separator is None:
        try:
            # importlib, so the deferral needs no PLC0415/import-outside-toplevel pair.
            module.Separator = importlib.import_module("audio_separator.separator").Separator
...
class PreprocessingManager:
    ...
    def _run_cpu_fallback(self, audio_path, yield_cb, error: Exception, stage="Vocal Separation"):
        ...
        try:
            ...
            # The separator built during the fallback is a CPU one, and it used to outlive
            # the failure that justified it: the device fields were restored but the object
            # was not, so a single transient error demoted the unit to CPU isolation for the
            # life of the process -- and the dashboard, reading the live separator, reported
            # CPU for an accelerator unit ever after. Dropping it makes the next run rebuild
            # on the unit's own device.
            self.separator = None
    ...
    def _run_isolation_pipeline(self, sep, audio_path, yield_cb=None):
        ...
        _log_isolation_complete_impl(unit_name, p_start, audio_dur, actual_provider[0] if actual_provider else "CPUExecutionProvider")
