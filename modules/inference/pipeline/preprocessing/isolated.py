...
#: Deadline for one UVR RPC. Without it WorkerChannel defaults to 0.0 -- no deadline at all
#: -- so a separator wedged in the vendor runtime held the channel lock forever and every
#: later request on that device queued behind it. That is the same wedge the ASR channel was
#: given a deadline for; this path had simply not been given one. Generous, because a
#: 20-minute source separates for minutes of real work on a slow device, and killing a slow
#: but healthy separation is worse than the hang it prevents.
_CALL_TIMEOUT_SEC = config_paths.float_env("UVR_WORKER_CALL_TIMEOUT_SEC", 1800.0, minimum=0.0)
...
class _SeparatorProbe:
    ...
    def providers(self) -> list[str]:
        """The ONNX execution providers the worker's real separator reported."""
        return list(self.onnx_execution_provider)
