...
def _resolve_amd_provider_name(available: list[str]) -> str | None:
    for prov in _get_amd_provider_order(available or []):
        return prov
    return None
...
def _wsl_dxg_fallback_message(rocdxg_present: bool) -> str:
    if rocdxg_present:
        return (
            "[UVR] AMD GPU fallback to CPU: /dev/dxg (WSL2) and librocdxg.so are present, "
            "but this Linux container still cannot use DirectML and lacks /dev/kfd for ROCm "
            "GPU execution. UVR will run on CPU. (available ORT providers: %s)"
        )
    return (
        "[UVR] AMD GPU fallback to CPU: /dev/dxg (WSL2) is present, but this Linux "
        "container cannot use DirectML and ROCm GPU execution still requires /dev/kfd. "
        "librocdxg.so may assist driver detection, but UVR will still run on CPU here. "
        "(available ORT providers: %s)"
    )


def _resolve_amd_fallback_message(kfd_present: bool, dxg_present: bool, rocdxg_present: bool) -> str:
    if not kfd_present and dxg_present:
        return _wsl_dxg_fallback_message(rocdxg_present)
