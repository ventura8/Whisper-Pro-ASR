...
def _vad_tuning(kwargs: dict) -> tuple[float, int, int]:
    """Resolve the VAD threshold, silence window and pad from either kwarg shape.

    model_manager sends the nested ``vad_parameters`` dict that faster-whisper's API takes,
    while this engine only ever read the flat ``vad_threshold``/``min_silence_duration_ms``
    names -- so the nested form fell through to the defaults and a configured VAD_THRESHOLD
    was silently ignored on INTEL-WHISPER. The hardcoded 0.35 default made that invisible:
    it is neither the configured value nor config's own 0.5, so Intel quietly ran a
    different VAD sensitivity than every other engine. Flat names win when both are given,
    since a caller naming this engine's own parameter is being specific.
    """
    nested = kwargs.get("vad_parameters") or {}
    threshold = kwargs.get("vad_threshold", nested.get("threshold", config.VAD_THRESHOLD))
    min_silence = kwargs.get("min_silence_duration_ms", nested.get("min_silence_duration_ms", config.VAD_MIN_SILENCE_DURATION_MS))
    pad = kwargs.get("speech_pad_ms", nested.get("speech_pad_ms", config.VAD_SPEECH_PAD_MS))
    return threshold, min_silence, pad
...
class IntelWhisperEngine:
    ...
    def _verify_device_executes(self, model_path: str) -> None:
        ...
        try:
            ...
            # escaped here aborted IntelWhisperEngine.__init__ instead of reaching
            # _fallback_to_cpu, turning a recoverable "this NPU cannot run this model" into
            # a dead service.
    ...
    def _get_speech_ts_safe(self, audio_data, **kwargs) -> list:
        try:
            threshold, min_silence, pad = _vad_tuning(kwargs)
    ...
    def apply_vad(self, audio_data: np.ndarray, **kwargs: Any) -> np.ndarray:
        ...
        try:
            v_threshold, v_min_silence, v_pad = _vad_tuning(kwargs)
    ...
    def detect_language(self, audio_data: Any) -> Tuple[str, float, List[Tuple[str, float]]]:
        ...
        # A path, not samples, is what gap_filling hands every engine: it extracts each gap
        # to its own file because an IsolatedEngine can only be given a path across the
        # process boundary. sanitize_audio on a str produced a nonsense array, so gap
        # language detection silently failed on the Intel path -- and every gap was then
        # skipped, which is the dropped-code-switched-legs defect gap-fill exists to fix.
        if isinstance(audio_data, str):
            audio_data = vad.decode_audio(audio_data)
