...
def worker_main(conn: Connection) -> None:
    ...
    for request in worker_runtime.iter_requests(conn):
        # Only real requests. A control message (the parent's "cancel") carries no "cmd",
        # so dispatching it produced a KeyError and an unsolicited error response on the
        # wire -- which the parent then read as the reply to its *next* call. Same guard
        # worker_runtime.serve applies.
        if not (isinstance(request, dict) and "cmd" in request):
            continue
