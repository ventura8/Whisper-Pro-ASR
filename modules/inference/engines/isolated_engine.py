...
#: A finite deadline for one worker call. Without it a call that never returns holds the
#: channel lock forever -- and because a channel is shared by every unit of that engine
#: type, that wedges the OTHER units too, while the scheduler goes on dispatching to them
#: because it sees free hardware and knows nothing about the lock.
#:
#: Observed on the Intel NUC (2026-09-06): INTEL-WHISPER serving a GPU unit and an NPU unit
#: through one channel logged "Blocked on lock for >5.0s during generation" and then nothing
#: for 2h40m, with 2 tasks active and 9 queued behind them at exactly the client's own
#: 15-minute timeout. WorkerChannel's default is 0.0, which it reads as "no deadline", and
#: this call site passed nothing -- whisperx_worker_client had a 1800s deadline all along,
#: which is why the same hang never appeared there.
#:
#: On expiry the channel shuts the worker down and the next call respawns it, so a hung
#: generate() costs one failed request instead of the whole service.
_CALL_TIMEOUT_SEC = float(os.environ.get("ASR_WORKER_CALL_TIMEOUT_SEC", "1800"))
...
class IsolatedEngine(BaseASREngine):
    ...
    def __init__(self, engine_type: str, model_id: str, unit: dict) -> None:
        ...
        #: The device the worker's engine actually settled on. Read once per load rather
        #: than per call: execution_status prefers a loaded engine's own answer over a
        #: prediction, and without this attribute the proxy had none to give -- so with
        #: isolation on, which is the default, the dashboard's ASR chip was always a
        #: configuration guess. That is precisely the case the chip exists to catch: an
        #: NPU engine that fell back to the CPU still read as "NPU, accelerated".
        self.device = self._worker_device()

    def _worker_device(self) -> Optional[str]:
        """Ask the worker what device its engine reports, tolerating a worker that cannot say."""
        try:
            return self._channel.call("engine_device", handle=self.handle)
        except worker_channel.WorkerError as exc:
            # Never fatal: this is reporting metadata, and an engine that loaded fine must
            # not be failed over a status field. execution_status falls back to prediction.
            logger.debug("[Isolated] %s worker could not report its device: %s", self.engine_type, exc)
            return None
    ...
    def _worker_env(self) -> dict[str, str]:
        ...
        """
        # importlib, not a deferred `from ... import`: the import must stay inside the
        # function to avoid an import cycle at module load, and a function-level import
        # statement needs two inline suppressions (ruff PLC0415, pylint
        # import-outside-toplevel) that this repository bans. A call is not an import
        # statement, so neither linter has anything to say about it.
    ...
    def _ensure_loaded(self) -> None:
        ...
        self.handle, self._generation = self._load()
        # Refreshed on reload: a respawned worker may settle on a different device -- an NPU
        # that failed warmup last time can succeed now, or the reverse -- and a stale value
        # would report the previous worker's answer as this one's.
        self.device = self._worker_device()
    ...
    def _iter_segments(self, stream: Iterator[dict]) -> Iterator[SegmentWrapper]:
        # try/finally, because callers routinely stop early: a cancelled task, an HTTP client
        # that disconnects, or any exception mid-transcription abandons this generator. Without
        # the finally the stream was never closed on those paths, so the worker kept producing
        # segments into a pipe nobody would read and the channel lock stayed held until the
    ...
    def detect_language_batch(self, audio_path: str, segment_count: int) -> Iterator[dict[str, Any]]:
        ...
        self._ensure_loaded()
        # Held and closed explicitly, exactly as _iter_segments does and for the same reason:
        # language detection is abandoned routinely (a cancelled task, a disconnected client),
        # and without the close the worker kept decoding windows into a pipe nobody would read
        # while the channel lock stayed held until the call deadline expired.
    ...
    def release_region_audio(self, audio_path: str) -> None:
        """Let the worker drop the samples it decoded for ``audio_path``'s region detection.

        Once per file, after its last chunk of regions: the samples are kept across chunks
        (and across a pause between them) on purpose, and released here so they do not
        outlive the request. By path, because the worker serves every unit of this engine
        type and another unit's file may be mid-detection. A dead worker has nothing left
        to release.
        """
        try:
            self._channel.call("release_region_audio", audio_path=audio_path)
        except worker_channel.WorkerError as exc:
            logger.info("[Isolated] Region audio release skipped for %s: %s", self.unit["id"], exc)
