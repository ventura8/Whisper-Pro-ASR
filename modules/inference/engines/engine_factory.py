...
def _resolve_torch_device(unit: dict) -> str:
    ...
    try:
        ...
        # Not just ImportError: importing torch loads its native libraries, which raise
        # OSError when a CUDA/ROCm .so is missing or ABI-mismatched and RuntimeError when
        # the runtime initialises but is unusable. Either way there is no torch to ask, and
        # the answer is the same CPU string -- unguarded they aborted engine construction.
        return "cpu"
...
#: NPU is deliberately absent, so _resolve_torch_device returns "cpu" for it. torch has no
#: NPU backend at all, and mapping it to the XPU probe put an NPU-assigned unit on the Intel
#: iGPU -- which is the unit the GPU-typed entry already serves, so both units' ASR
#: serialised on one device instead of proceeding in parallel. The same reasoning config.py
#: applies when the NPU cannot execute the Whisper IR: fall back to the CPU, and leave the
#: NPU free for the vocal-isolation preprocessing it is actually good at.
