...
def _pump_events(conn: Connection, generator: Iterator[Any], *, request_id: Any, cmd: Any) -> bool:
    ...
    It sits *after* the send, not before it. Checking first discarded an event that had
    already been pulled off the handler -- for a transcription that is the ``info`` event
    carrying the detected language and duration, so a cancel arriving in the first window
    lost the only metadata the parent was going to get, rather than merely stopping the
    stream early.
    """
