...
def _deadline_passed(deadline: Optional[float]) -> bool:
    """Whether a deadline exists and has elapsed. An absent deadline never passes."""
...
class WorkerChannel:
    ...
    def _wait_for_message(self, conn: Connection) -> bool:
        ...
        Returns False when the fixed deadline elapses, or when a concurrent shutdown is
        requested, so a hung worker cannot pin the lock forever.

        The wait is always chunked at ``_SHUTDOWN_POLL_INTERVAL_SEC`` rather than handed to
        ``poll`` as one long timeout. Sleeping for the whole deadline is the same bug the
        deadline was added to fix, one level up: with the 30-minute call timeout the
        isolated engine configures, a single ``poll(1800)`` would leave shutdown unnoticed
        for half an hour, so a stuck worker still pinned the interpreter at exit.
        """
        deadline = self._deadline()
        while True:
            if conn.poll(_SHUTDOWN_POLL_INTERVAL_SEC):
                return True
            if self._shutdown_requested.is_set() or _deadline_passed(deadline):
                return False

    def _deadline(self) -> Optional[float]:
        """The monotonic instant this call must give up at, or None when unbounded."""
        if self._call_timeout_sec <= 0:
            return None
        return time.monotonic() + self._call_timeout_sec
