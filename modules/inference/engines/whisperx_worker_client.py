...
# "generation" is bumped inside _teardown_worker every time the worker process
# is torn down as part of a lifecycle restart (timeout, error, or explicit
# shutdown) -- not directly when a new process is spawned. _teardown_worker is
# the single funnel-point for all paths that can destroy the worker
# (shutdown(), call()'s timeout/error paths, _ensure_worker()'s dead-process
# reap), so the bump is always authoritative. Callers that cache handles into
# the worker's `objects` pool (diarization.py's ALIGN_POOL/DIARIZE_POOL,
...
# (the new worker process starts with an empty one after teardown), so it
# must be treated as a miss rather than sent to the worker and fail there.
