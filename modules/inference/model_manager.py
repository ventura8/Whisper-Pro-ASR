...
from modules.inference import diarization, engine_factory, post_processing, preprocessing, scheduler, vad
from modules.inference.concurrency import _check_preemption, _get_current_task_info, model_lock_ctx
...
def _consume_segments(segments, info, task, *, diarize, min_speakers, max_speakers, hf_token, unit_id, processed_path):
    ...
    if diarize:
        try:
            ...
            for segment in segments:
                ...
                if hasattr(segment, "words") and segment.words is not None:
                    seg_dict["words"] = [
                        {"start": w.start, "end": w.end, "word": w.word, "probability": getattr(w, "probability", 1.0)}
                ...
                block = utils.format_single_srt_block(
                    idx=seg_idx, start_ts=segment.start, end_ts=segment.end, text=segment.text
                )
        ...
        for segment in segments:
            ...
            block = utils.format_single_srt_block(
                idx=len(results), start_ts=segment.start, end_ts=segment.end, text=segment.text
            )
            live_srt_blocks.append(block)
            scheduler.update_task_metadata(live_text="".join(live_srt_blocks), current_position=segment.end)
...
def unload_models():
    ...
    with _POOL_LOCK:
        ...
        logger.info(
            "[Engine] Reclamation complete. RAM: %s GB -> %s GB (Released: %d Whisper, %d UVR)",
            mem_before,
            mem_after,
            whisper_count,
            uvr_count,
