...
def for_decode(params: dict, was_auto_detect: bool) -> dict:
    ...
    ``batch_size`` travels only when the caller gave one: an engine that batches reads its
    own default when the key is absent, and a ``None`` would override that default.
    """
    options = _for_decode(params, was_auto_detect)
    if params.get("batch_size") is not None:
        options["batch_size"] = int(params["batch_size"])
    return options


def _for_decode(params: dict, was_auto_detect: bool) -> dict:
    return {
