...
def for_clips(model, processed_path: str, regions: list, preemption_check=None) -> list:
    ...
    Never raises for the detector's sake: a label lost is a region that becomes its own
    run, which is the pre-hysteresis behaviour, and losing the transcription over a lost
    label would be a far worse trade. What was labelled before the detector failed is kept.
    """
    if not regions:
        return []
...
def _collect(model, processed_path: str, regions: list, preemption_check) -> list:
    ...
    Only the detection is allowed to fail quietly, and a failure on a later chunk keeps the
    labels of the chunks before it: each is as measured as it would have been alone. The
    preemption callback runs between chunks, outside that guard and outside any stream:
    what it raises is the scheduler's business, and swallowing it here would keep labelling
    audio the task was told to give up; and it may block, which is only safe once the
    worker channel is free.
    """
    labelled: list = []
    try:
        for chunk in _chunks(regions):
            labelled.extend(_labelled(model, processed_path, chunk))
            (preemption_check or _nothing)()
    except _DetectionFailed:
        logger.warning("[ASR] Keeping the %d region label(s) measured before the detector failed", len(labelled))
    finally:
        _release_region_audio(model, processed_path)
    return labelled


def _nothing() -> None:
    """The check a caller without a scheduler passes: nothing to wait for."""


def _labelled(model, processed_path: str, chunk: list) -> list:
    """The spans of one chunk the detector could name a language for."""
    return [span for span in map(_labelled_span, _guarded_events(model, processed_path, chunk)) if span]


def _chunks(regions: list):
    """``regions`` in worker-call-sized pieces, in order."""
    for start in range(0, len(regions), DETECTION_CHUNK):
        stop = start + DETECTION_CHUNK
        yield regions[start:stop]


def _release_region_audio(model, processed_path: str) -> None:
    """Let a worker drop the file it decoded for the regions, once the file's detection is over.

    After the last chunk -- and on any early exit -- never between chunks, where the samples
    are what the next chunk needs; by path, so another unit's file in the same worker is
    left alone. An in-process engine has nothing to release. Its own failure is logged and
    not raised: it must never replace the detection's or the preemption check's exception,
    and a file left decoded in the worker is a cost, not an error.
    """
    release = getattr(model, "release_region_audio", None)
    if release is None:
        return
    try:
