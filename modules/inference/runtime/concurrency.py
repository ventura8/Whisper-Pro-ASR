...
def _wait_for_model_download(queued_added: bool) -> bool:
    ...
    """
    # `or 0`, not a .get default: the key is always present, so a stored None skipped the
    # default and put the literal "Downloading Model (None%)" on the dashboard.
    percent = int(model_provisioning.get_progress().get("percent") or 0)
