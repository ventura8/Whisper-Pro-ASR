...
def init_unit(unit):
    ...
    with _POOL_LOCK:
        ...
        # themselves fail, which would turn the real error into a NameError naming nothing.
        engine_type = getattr(config, "ASR_ENGINE", "")
        try:
            ...
            init_failures.record_init_failure(engine_type, unit, e)
...
def run_transcription(
    ...
    **kwargs,
    ...
    # The engines match the task exactly -- faster-whisper looks up the `<|task|>` token and
    # the Intel engine falls back to transcribe on anything outside its mapping -- so a
    # client's "Translate" is folded here, once, before any of them sees it.
    task = str(task or "transcribe").lower()
    ...
    with model_lock_ctx() as (model, unit_id):
        ...
        try:
            params = {
                ...
                # The caller's batch size, for the engine that batches (WhisperX); the
                # others drop it. Used to stop here, so every request ran at the default.
