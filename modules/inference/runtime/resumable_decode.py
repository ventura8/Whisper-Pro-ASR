...
def decode(model, processed_path: str, call: dict, clips: list | None, *, consume: dict, progress_window=None) -> tuple:
    ...
    while True:
        ...
        part, paused = _consume(segments, info, consume, progress_window)
        results.extend(part)
        spans = _decoded_so_far(clips, remaining)
        remaining = _pause(segments, consume, spans=spans, results=results, info=info, call=call, path=processed_path) if paused else []
        if not remaining:
            return results, info


def _consume(segments, info, consume: dict, progress_window) -> tuple:
    return model_segment_processing.consume_until_pause(
        segments,
        info,
        consume["task"],
...
def _pause(segments, consume: dict, *, spans: list | None, results: list, info, call: dict, path: str) -> list:
    """Release the worker, wait out the pause, and say what is left to decode.

    Empty when the pause came after the last clip's last segment: what is past it is audio
    the clips excluded, and decoding it would invent speech into that silence.
    """
    _close(segments)
    ...
    remaining = _remaining(spans, results, info, path, vad_filter=bool(call.get("vad_filter")))
    if remaining:
        logger.info("[ASR] Resuming: %d clip(s) left to decode", len(remaining))
    else:
        logger.info("[ASR] Resuming with nothing left to decode")
    return remaining
...
def _remaining(clips: list | None, results: list, info, processed_path: str, *, vad_filter: bool) -> list:
    """What is still undecoded after ``results``: clips past the last consumed segment.

    A whole-file call has no clips to trim. With ``vad_filter`` on, the remainder is scanned
    with the decode VAD's own settings -- the same speech the call would have compacted --
    and decoded as clips from the pause point on; with it off the caller asked for every
    sample decoded, so the remainder is one clip to the end of the file. Empty when nothing
    is left, and the caller returns then: an empty seek list must never reach the decoder,
    which reads it as the whole file.
    """
    ...
    if clips is not None:
        ...
        spans = _speech_after(processed_path, last_end, duration)
    else:
        spans = [{"start": last_end, "end": duration}]
    return _after(spans, last_end)
