...
def _extend(block: dict, region: dict, span) -> None:
    ...
    the middle was judged unsure and absorbed, on evidence that was never measured. Its
    seconds are not switch evidence either: only labelled speech says what language a
    block is in, so a one-second label followed by five unlabelled seconds is one second of
    evidence, not six.
    """
    block["regions"].append(region)
    if span:
        block["confidences"].append(float(span.get("confidence") or 0.0))
        block["labelled_seconds"] += float(region["end"]) - float(region["start"])
...
def _merge(blocks: list, file_language: str | None, total_speech: float) -> list:
    ...
    for block in blocks:
        if runs and _belongs_to(runs[-1], block, file_language, total_speech):
            ...
            runs.append({**block, "confidences": list(block["confidences"]), "regions": list(block["regions"])})
...
def _absorb(run: dict, block: dict, leading: bool = False) -> None:
    ...
    run["labelled_seconds"] += block["labelled_seconds"]
...
def _is_believable_switch(block: dict, file_language: str | None, total_speech: float) -> bool:
    ...
    if block["language"] == file_language:
        return True
    # Labelled seconds only: an unlabelled region continues a block but says nothing about
    # its language, so it cannot make a one-line label into a believable switch.
    enough = min(config.SEGMENT_RUN_MIN_SWITCH_SEC, config.SEGMENT_RUN_MIN_SWITCH_SHARE * total_speech)
    return block["labelled_seconds"] >= enough and _mean(block["confidences"]) >= config.LD_MIN_CONFIDENCE
