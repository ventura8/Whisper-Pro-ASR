...
def detect_language():
    ...
    responses:
      200:
        ...
        schema:
          type: object
          required: [detected_language, confidence, segments_processed, language, language_code]
          properties:
            detected_language:
              type: string
            ...
            language_name:
              type: string
              description: Lowercase full name of the detected language.
              example: "english"
            language_code:
              type: string
...
def transcribe():
      ...
      200:
        ...
        schema:
          ...
          properties:
            ...
            chunks:
              type: array
              description: Alias for 'segments' (Legacy compatibility).
              items:
                type: object
                properties:
                  text:
    ...
    """
    return _handle_asr_request()


def _handle_asr_request():
    """
    Coordinates the ASR lifecycle for the Flask route.
    """
    # Logic begins
...
def _calculate_and_log_stats(result, stats, request_start_time):
    """Refactored stats calculation to reduce locals."""
    ...
    stats['segment_count'] = len(result.get('segments', []))
    stats['text_len'] = len(result.get('text', ''))

    _log_request_stats(stats)


def _handle_transcription_error(err):
    """Centralized error handling for asr_endpoint."""
    status_code = 500
...
def _format_transcription_response(result, output_format, filename="transcription"):
    ...
    if output_format == 'json':
        # Add 'chunks' alias for legacy compatibility
    ...
    resp.headers["Content-Disposition"] = f'attachment; filename="{final_filename}"'
    return resp
...
def _log_detection_result(result, start_time):
    ...
    logger.info(
        "LD Completed | Lang: %s (%.1f%%) | Latency: %s | Segments: %d | Rank: %s",
        detected_lang, detected_conf, utils.format_duration(
            elapsed), segments, cand_str
    )
