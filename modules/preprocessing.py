...
including OpenVINO and CUDA backends, and handles both file-level and segment-level 
audio cleaning.
"""
import gc
import logging
...
class PreprocessingManager:
    ...
    def process_audio_file(self, file_path, yield_cb=None):
        """Perform vocal isolation on a media file from disk."""
        logger.debug("[Prep] Checking config: SEP=%s, LD_PREP=%s",
                     config.ENABLE_VOCAL_SEPARATION, config.ENABLE_LD_PREPROCESSING)

        if not (config.ENABLE_VOCAL_SEPARATION or config.ENABLE_LD_PREPROCESSING):
            logger.debug("[Prep] Vocal separation disabled, returning original.")
        ...
        try:
            self._init_separator()
            duration = utils.get_audio_duration(input_path)
            segment_size = config.VOCAL_SEPARATION_SEGMENT_DURATION

            if duration > segment_size:
                return self._run_segmented_isolation(input_path, segment_size, yield_cb, start)

            return self._run_single_pass_isolation(input_path, start)

        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("[Prep] Vocal isolation task failed: %s", e)
            return input_path

    def _run_segmented_isolation(self, input_path, segment_size, yield_cb, start_time):
        """Internal helper for segmented vocal isolation."""
        logger.info(
            "[Prep] File duration exceeds segment limit. Switching to segmented processing..."
        )
    ...
    def _isolate_and_write_segment(self, writer, input_path, start_frame, frames_to_read):
        ...
        sr = writer.samplerate
        segment_data, _ = sf.read(input_path, start=start_frame,
                                  frames=frames_to_read, always_2d=True)
        ...
        try:
            with self._lock:
                stems = self.separator.separate(tmp_path)

            vocal_path = self._identify_vocals(stems)
            if vocal_path and os.path.exists(vocal_path):
                processed = self._postprocess_segment(vocal_path, sr, frames_to_read)
                writer.write(processed)
            else:
                logger.warning("[Prep] Segment separation failed, using original.")
                writer.write(segment_data)
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            self._cleanup_stems(stems, None)
            gc.collect()

    def _run_single_pass_isolation(self, input_path, start_time):
        """Internal helper for single-pass vocal isolation."""
        stems = []
        vocal_path = None
        try:
            with self._lock:
            ...
            vocal_path = self._identify_vocals(stems)
            if vocal_path and os.path.exists(vocal_path):
                logger.info("[Prep] Isolated vocals finalized in %s.",
                            utils.format_duration(time.time() - start_time))
                return vocal_path

            return input_path
        finally:
            self._cleanup_stems(stems, vocal_path)
    ...
    def process_audio_segment(self, segment, sr=16000, yield_cb=None):
        """Clean an in-memory audio segment by oscillating to disk."""
        if not (config.ENABLE_VOCAL_SEPARATION or config.ENABLE_LD_PREPROCESSING):
            return segment
