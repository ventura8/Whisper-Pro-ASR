...
async def _await_shared_result_with_dashboard_task(
    ...
    *,
    ...
    Task registration and completion metadata run in worker threads (they call
    synchronous model_manager APIs). The shared-future wait itself runs on the
    event loop via asyncio.wrap_future so no worker thread is blocked for the
    duration of the leader's execution. A contextlib.ExitStack carries the
    early_task_registration context manager across the two thread calls.
    """
    exit_stack = contextlib.ExitStack()
    await anyio.to_thread.run_sync(lambda: _register_coalesced_follower(exit_stack, dedupe_key, filename, worker_context))
    result_payload = await _await_shared_result(shared_future)
    await anyio.to_thread.run_sync(lambda: _finalize_coalesced_follower(exit_stack, result_payload))
...
def _register_coalesced_follower(
    exit_stack: contextlib.ExitStack,
    dedupe_key: str,
    filename: str,
    worker_context: dict,
) -> None:
    """Enter task registration and set initial metadata for a coalesced follower.

    Runs in a worker thread (via anyio.to_thread.run_sync) so the synchronous
    model_manager/scheduler APIs can be called without blocking the event loop.
    The early_task_registration context manager is entered here and stored on
    exit_stack so _finalize_coalesced_follower can exit it in a subsequent thread
    call after the shared-future wait completes on the event loop.
    """
    routes_utils.apply_worker_context_from_dict(worker_context)
    exit_stack.enter_context(
        model_manager.early_task_registration(
...
def _finalize_coalesced_follower(
    exit_stack: contextlib.ExitStack,
    result_payload: "DetectResponsePayload",
) -> None:
    """Persist completion metadata and exit the task lifecycle context manager.

    Runs in a worker thread (via anyio.to_thread.run_sync) so the synchronous
    model_manager/scheduler APIs can be called without blocking the event loop.
    Must be called once after the shared-future wait resolves; exit_stack must be
    the same stack passed to _register_coalesced_follower for this follower.
    """
    try:
        if isinstance(result_payload, JSONResponse):
            if result_payload.status_code >= 400:
                fail_msg, fail_code = _json_response_failure(result_payload)
