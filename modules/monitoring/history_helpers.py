...
def _has_all_category_keys(daily_data: dict) -> bool:
    """Whether a stored day carries a usable breakdown for every category.

    Each value must be a mapping carrying numeric ``count`` and ``duration``, not merely a
    mapping and not merely present. Both callers go on to ``dict(...)`` these, and a
    hand-edited or half-migrated analytics file holding a number or a list under "asr"
    turned that into a TypeError during history load -- taking the whole analytics rebuild
    down instead of falling through to the legacy ASR backfill, which is exactly the path
    such a file needs.

    The field check is the other half of the same defect: a partial ``"asr": {}`` passed the
    isinstance test, so the merge branch ran and ``_add_uncategorised_remainder`` then did
    ``day["asr"]["count"] += ...`` on a mapping with no "count" -- a KeyError raised from
    inside the rebuild, again losing the whole file rather than backfilling it.
    """
    return all(_is_usable_category(daily_data.get(cat)) for cat in ("asr", "detectlang", "audio"))


def _is_usable_category(value: Any) -> bool:
    """Whether one category holds the numeric count and duration the merge arithmetic needs."""
    if not isinstance(value, dict):
        return False
    return all(_is_finite_number(value.get(field)) for field in ("count", "duration"))


def _is_finite_number(value: Any) -> bool:
    """A real count or duration: numeric, not a bool, and not NaN or infinity.

    NaN and inf pass isinstance(float) but are not usable totals. A stored NaN duration
    propagates through max() and += into every subsequent day, and serialises as bare
    ``NaN`` -- invalid JSON that the dashboard's parser rejects outright, so one poisoned
    day took the whole statistics endpoint down rather than being rebuilt.
    """
...
def _numeric(value: Any, default):
    """A stored total, or ``default`` when it is not a usable number.

    _has_all_category_keys validates the per-category totals but says nothing about a day's
    own count/duration, so a hand-edited file holding a string there reached max() and the
    subtraction in the merge -- a TypeError raised from inside the rebuild, losing the whole
    analytics file rather than backfilling the one bad day.
    """
    return value if _is_finite_number(value) else default
...
def _merge_overlapping_legacy_day(old_day: dict, rebuilt_day: dict) -> None:
    ...
    """
    # Coerced before use. _has_all_category_keys validates the per-category totals but says
    # nothing about the day's own count/duration, and a hand-edited file holding a string
    # there reached max() and the subtraction below -- a TypeError raised from inside the
    # rebuild, losing the whole analytics file rather than backfilling the one bad day.
    old_count = _numeric(old_day.get("count"), 0)
    old_dur = _numeric(old_day.get("duration"), 0.0)
    rebuilt_count = _numeric(rebuilt_day.get("count"), 0)
    rebuilt_dur = _numeric(rebuilt_day.get("duration"), 0.0)
...
def truncate_large_segments(task_data: dict) -> None:
    ...
    # A non-dict result is treated as absent. A failed task can carry ``result: None``, and
    # ``None.get`` is an AttributeError raised inside log_completed_task -- which catches
    # only KeyError/ValueError/TypeError, so it escaped and lost the whole history entry
    # for exactly the tasks whose record matters most.
