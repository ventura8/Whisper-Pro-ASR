...
def _fetch_intel_gpu_load():
    ...
    try:
        ...
        if any(
            t.get('unit_type') == 'GPU' and 'NVIDIA' not in t.get('unit_name', '')
...
def _fetch_single_intel_gpu_load(unit_id):
    ...
    try:
        ...
        active_tasks = stats.get('active_tasks', [])
        if any(
            (t.get('unit_id') == unit_id or
             (t.get('unit_type') == 'GPU' and 'NVIDIA' not in t.get('unit_name', '') and
              (_resolve_index(t.get('unit_id')) == idx or idx == 0))) and
            _is_task_using_accelerator(t, 'GPU')
            for t in active_tasks
        ):
            return 99
    except AttributeError:
    ...
    try:
        for pm in model_manager.PREPROCESSOR_POOL.values():
            ...
            if (pm.device_type == 'GPU' and 'NVIDIA' not in pm_unit_name and
                    _resolve_index(pm_unit_id) == idx and pm.lock.locked()):
                return 99
    except AttributeError:
        pass
...
def _fetch_single_npu_load(unit_id):
    ...
    # 1. Try Linux Accel Sysfs
    try:
        paths = glob.glob(f"/sys/class/accel/accel{idx}/device/utilization") + \
            glob.glob(f"/sys/class/drm/accel{idx}/device/utilization")
        if paths:
            with open(paths[0], 'r', encoding='utf-8') as f:
                val = int(f.read().strip())
                if val > 0:
    ...
    try:
        ...
        if any(
            (t.get('unit_id') == unit_id or
             (t.get('unit_type') == 'NPU' and (_resolve_index(t.get('unit_id')) == idx or idx == 0))) and
            _is_task_using_accelerator(t, 'NPU')
            for t in active_tasks
        ):
            return 100
    except AttributeError:
...
def _fetch_single_cuda_fallback(unit_id, idx):
    ...
    try:
        ...
        if any(
            (t.get('unit_id') == unit_id or
             (t.get('unit_type') == 'CUDA' and (_resolve_index(t.get('unit_id')) == idx or idx == 0))) and
            _is_task_using_accelerator(t, 'CUDA')
            for t in active_tasks
        ):
            return 99
    except AttributeError:
