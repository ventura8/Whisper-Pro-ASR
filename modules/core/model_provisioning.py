...
#: Set only when every required model is present and validated. is_ready() reports this,
#: and provision_models() treats it as "nothing left to do".
MODEL_READY = threading.Event()
#: Set when provisioning has stopped, successfully or not. This is what releases the
#: scheduler gate: a failed download must not queue tasks forever, but it must also not
#: claim the models are ready. The two were one event, so a failure reported is_ready()
#: True and made every later provision_models() call return early -- the retry a caller
#: asked for silently did nothing.
...
def should_gate_tasks():
    ...
    Either event opens the gate: success (MODEL_READY) or a failure that has given up
    (PROVISION_COMPLETE). Only the first of them means the models are actually there.
    """
    if not PROVISION_STARTED.is_set():
        return False
    return not (MODEL_READY.is_set() or PROVISION_COMPLETE.is_set())
...
def ensure_uvr_model(target_dir, temp_root):
    ...
    def _download():
        try:
            ...
            # The module's own tuple, not a second copy of it. Restating the list meant a
            # newly-handled failure mode had to be added twice, and the copy that was missed
            # would let that failure escape instead of falling through to the direct download.
...
def _fail(label, reason):
    ...
    # PROVISION_COMPLETE, not MODEL_READY: the gate opens so the real engine error surfaces
    # instead of an indefinite wait, while is_ready() stays False and a later
    # provision_models() call actually retries rather than returning early.
    PROVISION_COMPLETE.set()


def _provision_one(label, provision) -> bool:
    """Run one provisioning step, releasing the scheduler gate on any failure.

    The broad handler is the point. Anything ``_DOWNLOAD_ERRORS`` does not name used to
    escape ``provision_models`` entirely, leaving MODEL_READY and PROVISION_COMPLETE both
    unset -- so ``should_gate_tasks()`` returned True for the life of the process and every
    task queued forever behind a download that had already given up. From the outside that
    is indistinguishable from a very slow download.
    """
    try:
        if provision():
            return True
...
def provision_models(config):
    ...
    with _PROVISION_LOCK:
        ...
        for index, (label, provision) in enumerate(assets):
            ...
            if not _provision_one(label, provision):
                return False
