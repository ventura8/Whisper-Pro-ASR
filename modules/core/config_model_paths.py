...
def hybrid_is_usable(*, hybrid_engines: bool, model_id_by_engine: dict) -> bool:
    """Whether per-unit engines can actually be given weights each one can read.

    Hybrid mode hands an Intel unit INTEL-WHISPER, which reads an OpenVINO IR directory and
    nothing else. The per-engine table is populated only for the default model, so with a
    custom ASR_MODEL it is empty and model_id_for_engine falls through to MODEL_ID -- a
    CTranslate2 repo or path. The Intel unit was then handed CT2 weights, failed to load,
    and the operator's own model produced a half-dead pool. Without a table there is no
    hybrid; every unit runs the single resolved engine on the weights that were asked for.
    """
    return bool(hybrid_engines and model_id_by_engine)
