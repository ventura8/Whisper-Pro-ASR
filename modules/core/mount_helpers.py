...
def get_custom_mount_points() -> list[str]:
    """Discover the non-system mount points this container has, from /proc/mounts.

    Discovery only. Whether these become approved roots for caller-supplied ``local_path``
    reads is the caller's decision -- see ``config_paths._auto_approved_mount_roots``, which
    applies ``WHISPER_AUTO_APPROVE_MOUNTS`` and logs what it admitted. This docstring used to
    say "to automatically approve volumes", which described a policy that no longer lives
    here and read as though the grant were unconditional.
    """
