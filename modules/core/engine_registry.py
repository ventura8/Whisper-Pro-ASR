...
#: Where each segregated runtime lives when its environment variable is unset. The worker
#: itself already defaults to this path (see whisperx_worker._activate_isolated_lib_path), so
#: without the same default here the two disagreed: an image that ships WhisperX at the
#: standard location but does not export WHISPERX_LIB_PATH reported the engine as missing and
#: fell back to another one, while the worker would have loaded it perfectly well.
ENGINE_LIB_PATH_DEFAULT: dict[str, str] = {
    ENGINE_WHISPERX: "/app/libs/whisperx",
}
...
def _module_is_in_segregated_lib(engine: str, module: str) -> bool:
    """Whether ``module`` is present in the engine's own out-of-path install directory."""
    lib_path = os.environ.get(ENGINE_LIB_PATH_ENV.get(engine, ""), "") or ENGINE_LIB_PATH_DEFAULT.get(engine, "")
