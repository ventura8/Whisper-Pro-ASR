...
#: Unit types vocal isolation can run on with hardware acceleration. UVR is ONNX Runtime,
#: which has a provider for each of these, so such a unit earns its place in the pool even
#: when the ASR engine cannot decode on it. Mirrors config.ACCELERATED_PREPROCESS_DEVICES;
#: restated here because this module deliberately imports no configuration -- everything it
#: decides is a pure function of its arguments.
_UVR_ACCELERATED_TYPES = frozenset({"CUDA", "GPU", "NPU", "AMD"})
...
def _refuse_hybrid_for_explicit_engine(state: dict, *, possible: bool, logger) -> None:
    """Turn hybrid off because the operator named an engine, and say so when it conflicts.

    An explicit ASR_ENGINE is the narrower instruction and wins. Hybrid mode assigns each
    unit the engine native to its silicon, so with both set the named engine was silently
    discarded on every unit -- ASR_ENGINE=OPENAI-WHISPER plus HYBRID_ENGINES=true ran
    Faster-Whisper and Intel-Whisper and nothing the operator asked for, while the banner
    still reported the request as honoured.
    """
    state["hybrid"] = False
    if not possible or (state["hybrid_env"] or "").strip().lower() not in _FORCE_WORDS:
        return
    logger.warning(
        "HYBRID_ENGINES is set but ASR_ENGINE=%s was requested explicitly; the named engine runs on every unit.",
        state["engine"],
    )
    state["parts"].append(f"hybrid -> off (explicit {state['engine']})")
...
def _resolve_hybrid(state: dict, *, hardware_units: list, logger) -> None:
    ...
    if state["engine_source"] == "explicit":
        _refuse_hybrid_for_explicit_engine(state, possible=possible, logger=logger)
        return
    ...
    _report_hybrid_outcome(state, possible=possible, logger=logger)


def _report_hybrid_outcome(state: dict, *, possible: bool, logger) -> None:
    """Say what hybrid resolved to, so half the available throughput is never silent."""
    if state["hybrid"]:
        logger.info(
...
def _keep_only_drivable_units(state: dict, *, hardware_units: list, logger) -> None:
    """Keep a unit that can accelerate *either* stage; drop one that can accelerate neither.

    The rule is per-unit, not per-engine: a unit earns its place in the pool if the resolved
    ASR engine can decode on it **or** vocal isolation can run on it accelerated. Those are
    different runtimes -- CTranslate2 for decoding, ONNX Runtime for UVR -- and they reach
    different silicon, so a unit that is useless to one is routinely valuable to the other.

    This replaces a rule that dropped every unit the ASR engine could not drive. The
    justification for that was the CPU degradation being silent; it no longer is, because
    ``modules/monitoring/execution_status.py`` reports each unit's real execution device and
    labels a unit whose ASR landed on the CPU as a fallback. What the old rule cost was a
    whole scheduler slot: an Intel GPU that CTranslate2 cannot touch still runs a complete
    task -- isolation on the iGPU, decoding on the CPU -- so a hybrid NVIDIA+Intel host ran
    one task at a time where it could run two.

    A unit that can accelerate neither stage is still dropped, because it is a CPU slot
    wearing an accelerator's name: an AMD unit on an image with no ROCm ONNX Runtime, say.
    The trade for the units that stay is deliberate: a task landing on one decodes on the
    CPU, so per-task latency is worse while total throughput is better.
    ``_align_device_with_engine`` still corrects the reported ASR DEVICE, so nothing claims
    an accelerator it is not using.
    """
    if state["hybrid"] or len(hardware_units) <= 1:
        return
    drivable = _drivable_units(state["engine"], hardware_units)
    narrowed = _narrow_to_useful_units(state, hardware_units, drivable, logger=logger)
    _report_kept_cpu_fallbacks(state, hardware_units, drivable, narrowed=narrowed, logger=logger)


def _report_kept_cpu_fallbacks(state: dict, hardware_units: list, drivable: list, *, narrowed: bool, logger) -> None:
    """Name the units that stayed in the pool but will decode on the CPU.

    Only meaningful when the pool was narrowed. The CPU-fallback branch *replaces*
    hardware_units, and the substitute CPU unit is by definition not in ``drivable`` -- so
    reporting unconditionally announced the replacement itself as an accelerator unit that
    had fallen back to the CPU, which is both untrue and circular.
    """
    if not narrowed:
        return
    _report_cpu_fallback_units(state, [unit for unit in hardware_units if unit not in drivable], logger=logger)


def _narrow_to_useful_units(state: dict, hardware_units: list, drivable: list, *, logger) -> bool:
    """Keep the units that accelerate something; fall back to the CPU when none do.

    Returns whether the existing pool was narrowed (True) or replaced wholesale by a CPU
    unit (False), which the caller needs in order to describe the outcome correctly.
    """
    useful = [unit for unit in hardware_units if unit in drivable or _uvr_can_accelerate(unit)]
    if not useful:
        _fall_back_to_a_cpu_unit(state, hardware_units, logger=logger)
        return False
    _narrow_pool(
        state,
        hardware_units,
        useful,
        logger=logger,
        message="Kept %s in the scheduler pool; dropped units that can accelerate neither decoding nor vocal isolation: %s",
        part="pool -> units that accelerate ASR or UVR",
    )
    return True


def _fall_back_to_a_cpu_unit(state: dict, hardware_units: list, *, logger) -> None:
    """Serve on an explicit CPU unit when nothing present accelerates either stage.

    The scheduler cannot run with an empty pool, and a host whose only accelerator is
    unusable -- an AMD card on an image with no ROCm ONNX Runtime, say -- must still serve
    requests and must still show a unit on the dashboard. A named CPU unit is the honest
    thing to show: it says what will actually execute, where leaving the unusable
    accelerator in the pool would have the dashboard name silicon that never runs anything.
    """
    logger.warning(
        "No detected unit can accelerate decoding or vocal isolation with %s; serving on the CPU.",
        state["engine"],
    )
    hardware_units[:] = [{"type": "CPU", "id": "CPU", "name": "Host CPU"}]
    state["parts"].append("pool -> CPU (no unit accelerates ASR or UVR)")


def _uvr_can_accelerate(unit: dict) -> bool:
    """Whether vocal isolation can run on this unit with hardware acceleration."""
    return str(unit.get("type", "")).strip().upper() in _UVR_ACCELERATED_TYPES


def _report_cpu_fallback_units(state: dict, undrivable: list, *, logger) -> None:
    """Say which pooled units will decode on the CPU, so the kept slot is not mistaken for
    an accelerated one."""
    if not undrivable:
        return
    logger.info(
        "%s cannot drive %s; those units stay in the pool for preprocessing and decode on the CPU. "
        "Their dashboard execution status reports the fallback.",
        state["engine"],
        _unit_ids(undrivable),
    )
    state["parts"].append(f"pool -> kept {_unit_ids(undrivable)} (ASR on CPU, preprocessing on the unit)")
...
def _align_device_with_engine(state: dict, *, hardware_units: list, logger) -> None:
    """DEVICE must never name hardware the resolved engine cannot actually drive.

    Two outcomes, and picking between them is the point. If the pool still holds a unit the
    engine CAN drive, DEVICE moves there; only when nothing is drivable does it fall to the
    CPU.

    The CPU case is the Intel-only host running the default engine, where CTranslate2 has
    no OpenVINO backend at all: DEVICE stayed "NPU", the banner announced an NPU, and every
    request decoded on the CPU. The unit stays in the pool regardless, because it is still
    a perfectly good place to run vocal isolation -- UVR is ONNX Runtime and reaches
    devices CTranslate2 never will. Only the ASR device claim is corrected, and loudly.

    The drivable case is the one that used to be missing. _restrict_pool_to_intel drops the
    CUDA units without moving DEVICE, and _keep_only_drivable_units -- the only other stage
    that moves it -- does nothing when everything left is already drivable. So on a
    CUDA+Intel host with ASR_ENGINE=INTEL-WHISPER, DEVICE fell through to "CPU" while tasks
    were dispatched to the Intel GPU and genuinely ran there (create_engine routes on the
    unit, not on DEVICE). The banner read "OpenVINO (CPU)" for work happening on the iGPU.
    """
    if engine_registry.engine_supports_unit(state["engine"], state["device"]):
        return
    previous = state["device"]
    drivable = _drivable_units(state["engine"], hardware_units)
    state["device"] = str(drivable[0].get("type", "CPU")) if drivable else "CPU"
    state["asr_device_env"] = state["device"]
    state["parts"].append(f"device -> {state['device']} ({state['engine']} cannot drive {previous})")
    if state["device"] == "CPU":
        logger.warning(
        ...
        return
    logger.info(
        "ASR_DEVICE moved from %s to %s: %s cannot drive %s, and %s is in the pool.",
        previous,
        state["device"],
        state["engine"],
        previous,
...
def resolve_engine_and_pool(
    *,
    ...
    _align_device_with_engine(state, hardware_units=hardware_units, logger=logger)
