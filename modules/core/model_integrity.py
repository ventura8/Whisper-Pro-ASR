...
def _ov_xml_bin_pair_valid(path: Path, prefix: str, min_bin_bytes: int) -> bool:
    ...
    # verify_file_min_size rather than is_file()/stat(): a dangling symlink or a path on a
    # dismounted volume raises OSError from stat, and this runs during provisioning where
    # the answer wanted is "not valid", not a traceback.
    if not verify_file_min_size(xml_file, 1):
