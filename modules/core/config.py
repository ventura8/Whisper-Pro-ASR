...
#: engine_is_installed, not `isdir(WHISPERX_LIB_PATH)`. The directory existing says only
#: that a path was created; the shared probe also accepts an importable whisperx and, for
#: the lib-path case, requires the package itself to be inside it. The weaker check
#: reported an empty or half-copied /app/libs/whisperx as a working WhisperX install.
WHISPERX_AVAILABLE = engine_registry.engine_is_installed(engine_registry.ENGINE_WHISPERX)
...
# Compared against the operator's ORIGINAL setting, not the resolution's own output.
# ASR_DEVICE_ENV was rebound from _ENGINE_RESOLUTION four lines up, and resolution moves the
# two together -- so `DEVICE != ASR_DEVICE_ENV` was almost always false and the environment
# was never updated. Workers inherit ASR_DEVICE from the environment and skip detection, so
# they were told the device the operator asked for rather than the one resolution settled on.
# .strip() before .upper(), matching ASR_ENGINE_ENV above: a trailing space from an env
# file left "CUDA " unequal to the resolved "CUDA", so the comparison below thought the
# device had moved and rewrote ASR_DEVICE on every start.
_ASR_DEVICE_REQUESTED = (os.environ.get("ASR_DEVICE") or "AUTO").strip().upper()
if DEVICE != _ASR_DEVICE_REQUESTED and DEVICE in ("CUDA", "AMD", "GPU", "NPU", "CPU"):
...
def get_temp_dir(required_bytes=0):
    ...
    Returns ``TEMP_DIR`` when it has ``TEMP_DIR_MIN_FREE_BYTES`` of headroom plus
    ``required_bytes``, and ``PERSISTENT_TEMP_DIR`` otherwise -- including when the free-space
    check itself raises ``OSError`` (an unmounted tmpfs, a path that has gone away), because
    an unanswerable question about the RAM disk is treated as a reason not to use it.

    It does **not** raise, and it does **not** check the persistent directory against the
    same threshold: the fallback is returned unconditionally, so a caller that needs
    ``required_bytes`` guaranteed must still handle ENOSPC. That is deliberate -- the
    persistent volume is the last resort, and failing here would leave no path at all.
    """
...
def _resolve_hybrid_after_model_paths() -> tuple[bool, str]:
    """Settle HYBRID_ENGINES and the resolution text, now that the weights table is known.

    Applied here rather than during engine resolution because it depends on the per-engine
    weights table, which is only resolved above. Hybrid mode hands an Intel unit
    INTEL-WHISPER, and that engine reads an OpenVINO IR directory and nothing else; the
    table is populated only for the default model, so with a custom ASR_MODEL the Intel unit
    was handed CTranslate2 weights and failed to load, leaving half the pool dead.

    A function returning both values, rather than two module-level names reassigned inside
    an `if`: reassignment made pylint classify them as mutable module variables that must be
    snake_case, and they are read as constants everywhere else. One assignment each, below.
    """
    hybrid = _ENGINE_RESOLUTION["HYBRID_ENGINES"]
    resolution = _ENGINE_RESOLUTION["ASR_ENGINE_RESOLUTION"]
    if hybrid and not config_model_paths.hybrid_is_usable(hybrid_engines=True, model_id_by_engine=MODEL_ID_BY_ENGINE):
        logger.warning(
            "HYBRID_ENGINES is off for ASR_MODEL=%s: per-unit engines need a weights set per engine, "
            "and only the default model has one. Every unit runs %s on the requested weights.",
            ASR_ENV,
            ASR_ENGINE,
        )
        return False, f"{resolution} | hybrid -> off (custom ASR_MODEL has no per-engine weights)"
    return hybrid, resolution


HYBRID_ENGINES, ASR_ENGINE_RESOLUTION = _resolve_hybrid_after_model_paths()
...
if os.environ.get("VERIFY_RUNTIME", "true").lower() == "true" and DEVICE == "NPU" and ASR_ENGINE == engine_registry.ENGINE_INTEL_WHISPER:
    ...
    if not _npu_ok:
        ...
        ASR_DEVICE_ENV = "CPU"
        # The pool is deliberately left exactly as it is.
        #
        # There used to be a branch here that removed the NPU while keeping the other units.
        # It could never run: reaching this point requires DEVICE == "NPU", and every route
        # to that leaves an NPU-ONLY pool. ASR_DEVICE=NPU makes
        # _restrict_pool_to_requested_device narrow to the NPU before this probe, and
        # ASR_DEVICE=AUTO only resolves to NPU when nothing outranks it (CUDA > AMD > GPU >
        # NPU), which again means no other unit is present. Measured across every reachable
        # shape: the removal never fired once. It was dead code describing a scenario the
        # resolution order makes impossible.
        #
        # What actually protects a task dispatched to the NPU is IntelWhisperEngine's own
        # _verify_device_executes: it warms up on the NPU, sees the Level Zero failure, and
        # moves that engine to the CPU itself. That is the behaviour validated on the NUC.
        #
        # Retyping the unit to "CPU" was tried and reverted: engine_factory routes by unit
        # type, so a CPU-typed unit made it degrade INTEL-WHISPER to Faster-Whisper and hand
        # the OpenVINO IR directory to CTranslate2, which reported
        #
        #     RuntimeError: Unable to open file 'model.bin' in model './model_cache/whisper-openvino'
        #
        # and failed all 9 accuracy tests on the NUC (2026-09-06, ASR_DEVICE=NPU). Worse, that
        # is the path whose corruption handler purges what it decides is a bad CT2 model --
        # i.e. it can delete the OpenVINO weights. It is the same trap _restrict_pool_to_intel
        # is written to avoid.
        logger.error("ASR falls back to the CPU; the NPU stays available for UVR preprocessing.")
...
VAD_THRESHOLD = config_paths.float_env("VAD_THRESHOLD", 0.5, minimum=0.0, maximum=1.0)
