...
def _probe_duration_safe(file_path: str, flags: list[str] | None, check_output_fn: Callable[..., str]) -> float | None:
    ...
    # Broad by necessity, and deliberately so. The probe runs through an injected callable,
    # and what a failing probe raises is that callable's business: process_exec raises its
    # own CommandExecutionError, subprocess raises CalledProcessError/TimeoutExpired, a
    # missing binary raises OSError, unparseable output raises ValueError, and a corrupt
