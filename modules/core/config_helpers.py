...
#: Values that read as "off" wherever this module parses a boolean environment variable,
#: matching config.py's ISOLATE_ENGINES/ISOLATE_PREPROCESSING parsing.
...
def is_worker_context() -> bool:
    ...
    Parsed by value. Any non-empty string used to mean "yes", so WHISPER_WORKER_CONTEXT=0
    -- the obvious way to say "not a worker" -- put the API process into worker mode, where
    detect_hardware skips probing entirely and reports a CPU-only host.
    """
    return (os.environ.get("WHISPER_WORKER_CONTEXT") or "").strip().lower() not in ("", *_FALSE_WORDS)
