...
def run_detection_pipeline(model, processor, audio_path, yield_cb=None):
    ...
    """
    try:
        segments = _collect_active_speech_segments(audio_path, yield_cb=yield_cb)
        ...
        if is_val_or_run or is_sf_err:
            ...
            segments = _sample_audio_segments(audio_path)
        else:
            raise err
    ...
    logger.info(
        "[LD] Analyzing %d high-RMS speech segments for identification.", len(segments))

    # Phase 1: Probability Extraction
    segment_probs_list = _detect_languages_from_segments(model, processor, segments)

    # Phase 2: Aggregation (Squared Weighting)
    avg_probs = _aggregate_language_probs(segment_probs_list)
...
def _detect_languages_from_segments(model, processor, segments):
    ...
    if not lang_to_id:
        ...
        return _detect_languages_fallback(model, processor, segments)
    ...
    for i, segment in enumerate(segments):
        ...
        prog_bar = '=' * int(20 * pct / 100) + '>' + '.' * \
            (20 - int(20 * pct / 100) - 1)
        logger.info("[LD Progress] %5.1f%% | Segment %d/%d | [%s]",
                    pct, i + 1, total, prog_bar[:20])

        probs = _detect_single_segment_probs(
...
def _detect_single_segment_probs(model, processor, segment, lang_ids, id_to_lang):
    """Extract softmax probabilities for the first generated token across all languages."""
    feats = processor(segment, sampling_rate=16000,
...
def _detect_languages_fallback(model, processor, segments):
    """Greedy decoding fallback for identification on non-standard models."""
    probs = []
    for segment in segments:
        ids = model.generate(
            processor(segment, sampling_rate=16000,
                      return_tensors="pt").input_features,
            max_new_tokens=1,
            num_beams=1
        )
...
def _sample_audio_segments(audio_path):
    """Fixed-offset sampling used when smart sampling fails."""
    info = sf.info(audio_path)
    duration = info.duration
    num_segments = max(3, min(9, int(duration / 120)))
    offsets = np.linspace(0, max(0, duration - 30), num_segments)
    return [_read_segment(audio_path, o, 30, info.samplerate) for o in offsets]
...
def _collect_active_speech_segments(audio_path, yield_cb=None):
    ...
    for i in range(target):
        start = i * zone_size
        segment = _search_or_fallback_zone(
    ...
    if config.SMART_SAMPLING_SEARCH:
        ...
        for retry in range(3):
            ...
            if offset + 30 > info.duration:
                break
            raw = _read_segment(audio_path, offset, 30, info.samplerate)

            # Diagnostic RMS check
            if np.sqrt(np.mean(raw**2)) >= 0.005:
                return pm.process_audio_segment(raw, yield_cb=yield_cb) if pm else raw
...
def _read_segment(audio_path, start_sec, duration_sec, file_sr):
