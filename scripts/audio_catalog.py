...
# Resolved after the path bootstrap above: this file runs as a script, so `scripts` is not
# an importable package until ROOT is on sys.path. importlib rather than an out-of-position
# `from ... import`, which would need the E402/wrong-import-position pair this repo bans.
FFMPEG_TIMEOUT_SEC = importlib.import_module("scripts.audio_matrix.render").FFMPEG_TIMEOUT_SEC
...
def _table(header: list[str], rows: list[list[str]]) -> list[str]:
    # The separator row is padded like the header. markdownlint's MD060 enforces one
    # consistent table style per document, and the unpadded "|---|---|" form contradicts the
    # padded header rows this same function emits -- so the generated catalog failed the
    # repo's own markdown gate.
...
def _longform_lines(spec: dict) -> list[str]:
    ...
    return [
        ...
        + _languages_label(spec.get("languages") or [])
...
def _languages_label(languages: list) -> str:
    """How many languages a long-form clip holds, and which.

    An explicit empty list is a spec the generator refuses to build (it asks for no
    languages), and the catalog has to say so rather than fail on ``languages[0]``.
    """
    if not languages:
        return "no languages (the spec names none, so nothing renders)"
    if len(languages) == 1:
        return f"one language ({languages[0]})"
    return f"{len(languages)} languages ({', '.join(languages)})"
