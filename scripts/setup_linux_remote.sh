...
if [ "$VERIFY_ONLY" != true ]; then
	...
	# only the private key had been kept.
	PUBKEY="$(cat "${KEY}.pub")"
	hdr "One block to paste on the Linux machine"
	# The group loop below ends with `|| true` deliberately. A for-loop's exit status is its
	# last iteration's, so on a host with no `video` group the final `getent` returned 1, the
	# pasted block's own `set -e` killed it there, and READY -- the line that tells the
	# operator which username to hand back -- was never printed.
	cat <<EOF
