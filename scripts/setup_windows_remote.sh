...
#   --distro NAME WSL distro the setup command selected (its READY line prints it).
#                 Default: Ubuntu. Every WSL probe below, and the remote_validate.sh command
#                 printed at the end, use this one -- a bare `wsl -e` silently targets the
#                 host's DEFAULT distro, which is not necessarily the one that was set up.
...
if [ "$VERIFY_ONLY" != true ]; then
	...
	# The key travels base64-encoded and is decoded into a variable inside PowerShell rather
	# than spliced into PowerShell source. A public key is not a secret, but it IS
	# caller-supplied text concatenated into a command line that a human then pastes into a
	# shell -- a quote or a $( ) in an unusual key's comment field would break out of the
	# single-quoted literal, here and again in the UAC relaunch the .ps1 builds from it.
	KEY_B64="$(printf '%s' "$PUBKEY" | base64 | tr -d '\n')"
	ONELINER="powershell -NoProfile -ExecutionPolicy Bypass -Command \"\$b=[Convert]::FromBase64String('${PS_B64}'); \$m=New-Object IO.MemoryStream(,\$b); \$g=New-Object IO.Compression.GzipStream(\$m,[IO.Compression.CompressionMode]::Decompress); \$s=(New-Object IO.StreamReader(\$g)).ReadToEnd(); \$f=Join-Path \$env:TEMP 'whisper_setup.ps1'; Set-Content -Path \$f -Value \$s -Encoding UTF8; try { & \$f -PublicKeyBase64 '${KEY_B64}' } finally { Remove-Item -Force -ErrorAction SilentlyContinue \$f }\""
	LEN=${#ONELINER}
	cat <<EOF
...
type, opens the firewall, and checks WSL2 + Docker. Its last line (READY ...) reports both your
username and the WSL distro it configured -- tell me both, and pass the distro to this
...
# Every probe names the distro explicitly. A bare `wsl -e` runs against the host's DEFAULT
# distro, which need not be the one setup_windows_remote.ps1 selected and reported -- so on a
# machine with more than one, these checks could pass against a distro that has no Docker
# integration while the configured one was never tested (or the reverse).
if run "wsl -d ${DISTRO} -e bash -lc \"echo ok\"" 2>/dev/null | grep -q ok; then
	note "wsl: reachable ($(run "wsl -d ${DISTRO} -e bash -lc \"grep PRETTY_NAME /etc/os-release | cut -d= -f2\"" 2>/dev/null | tr -d '\"'))"
else
...
note "nvidia in wsl: $(run "wsl -d ${DISTRO} -e bash -lc \"nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo none\"" 2>/dev/null)"
note "free space: $(run "wsl -d ${DISTRO} -e bash -lc \"df -BG --output=avail / | tail -1\"" 2>/dev/null | tr -d ' ')"
...
=== Ready ===
  Validate with:
    ...
    scripts/remote_validate.sh ${USERNAME}@${HOST} --wsl ${DISTRO} --full
