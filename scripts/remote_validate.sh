...
#                   when a separation actually runs, so a default run proves nothing
#                   about it however green it is.
...
# validation plan (L3, L8, N8) differ from their neighbours only by an ASR_ISOLATE_* value,
...
while [ $# -gt 0 ]; do
	...
	--device)
		...
		case "$DEVICE" in AUTO | CPU | GPU | NPU | CUDA | AMD) ;;
		*)
			echo "unknown --device '${DEVICE}' (AUTO|CPU|GPU|NPU|CUDA|AMD)" >&2
			exit 2
			;;
		esac
		shift 2
	...
	--wsl)
		...
		case "${2:-}" in "" | -* | *@* | local | localhost) shift ;; *)
			WSL_DISTRO="$2"
			shift 2
			;;
		esac
	...
	--engine)
		...
		case "$ENGINE" in AUTO | FASTER-WHISPER | INTEL-WHISPER | OPENAI-WHISPER | WHISPERX) ;;
		*)
			echo "unknown --engine '${ENGINE}' (AUTO|FASTER-WHISPER|INTEL-WHISPER|OPENAI-WHISPER|WHISPERX)" >&2
			exit 2
			;;
		esac
		shift 2
		;;
	--preprocess)
		PREPROCESS="${2:?--preprocess needs a value}"
		case "$PREPROCESS" in AUTO | CPU | GPU | NPU | CUDA | AMD) ;;
		*)
			echo "unknown --preprocess '${PREPROCESS}' (AUTO|CPU|GPU|NPU|CUDA|AMD)" >&2
			exit 2
			;;
		esac
		shift 2
...
# The value that will reach the test client gets the same check --timeout gives its own:
# anything but whole seconds greater than zero would otherwise fail twenty minutes in.
# After the loop, not before it, so an unusable REAL_ASR_TIMEOUT in the environment is
# overridden by a valid --timeout rather than rejecting the run the flag was meant to fix.
...
# every ssh_run stage cd'd into the real $HOME/whisper-pro-asr and found nothing there.
# Resolved once by init_remote_home below and reused by both syncs.
#
# The memoisation used to live inside remote_dest, which is only ever called as
# "$(remote_dest)" -- a command substitution runs in a SUBSHELL, so the assignment to
# REMOTE_HOME was discarded every time and each call paid a fresh SSH round trip to a
# Windows host through wsl. Resolving it in the parent shell before the sync stages is what
# actually makes it once.
REMOTE_HOME=""
#: The Windows profile directory as WSL sees it, e.g. /mnt/c/Users/ventu. Resolved once from
#: the host rather than assumed: the profile is not always C:\Users\<login name> -- a renamed
#: account, a roaming profile or a redirected drive all break that guess, and the failure is a
#: copy into a path that does not exist, reported as a bash "not found" for the script itself.
WIN_PROFILE_WSL=""
...
init_win_profile() {
	...
	# levels of quoting, and the mangled form silently produced a path with the separators
	# left as backslashes.
	local drive rest
	drive="${win_path:0:1}"
	rest="${win_path:2}"
	WIN_PROFILE_WSL="/mnt/${drive,,}${rest//\\//}"
}
...
remote_dest() {
	...
	printf '%s/whisper-pro-asr' "$REMOTE_HOME"
}
...
ssh_run() {
	...
	tmp="$(mktemp)"
	# A unique name per invocation. The fixed "wpa_cmd.sh" is written into the Windows home
	# directory, so two runs against one host -- a matrix row and a hand-run command, or two
	# operators -- overwrote each other's script mid-flight and executed the wrong one.
	remote_name="wpa_cmd_$$_${RANDOM}.sh"
	...
	scp -i "$KEY" "${SSH_OPTS[@]}" -q "$tmp" "${REMOTE}:${remote_name}" || {
		rm -f "$tmp"
		return 1
	}
	rm -f "$tmp"
	...
	local rc=0
	ssh -i "$KEY" "${SSH_OPTS[@]}" "$REMOTE" \
		"wsl -d ${WSL_DISTRO} -e bash -c \"sed -i 's/\\r$//' ${WIN_PROFILE_WSL}/${remote_name}; bash ${WIN_PROFILE_WSL}/${remote_name}\"" || rc=$?
	# Removed whether the command succeeded or failed, but only after it has finished with the
	# file -- and the command's own status is what this function returns, so the cleanup
	# cannot mask a failure.
	ssh -i "$KEY" "${SSH_OPTS[@]}" "$REMOTE" \
		"wsl -d ${WSL_DISTRO} -e rm -f ${WIN_PROFILE_WSL}/${remote_name}" >/dev/null 2>&1 || true
	return "$rc"
}
...
if [ "$WSL" = true ]; then
	...
	scp -i "$KEY" "${SSH_OPTS[@]}" -q "${REPO_ROOT}/scripts/audit_hardware_windows.ps1" \
		"${REMOTE}:${AUDIT_REMOTE_NAME}" || die "could not copy the Windows audit script"
...
if [ "$LOCAL" = true ]; then
	...
	# Before the first "$(remote_dest)", so the resolved value survives into the parent shell.
	init_remote_home
...
if [ "$FIXTURES" = true ] && [ "$LOCAL" != true ]; then
	...
	if [ -d "${REPO_ROOT}/test_data/audio_matrix" ]; then
		...
		rsync -az --info=progress2 ${RSYNC_PATH_ARGS[@]+"${RSYNC_PATH_ARGS[@]}"} \
			-e "ssh -i $KEY ${SSH_OPTS[*]}" \
			"${REPO_ROOT}/test_data/audio_matrix/" "${REMOTE}:$(remote_dest)/test_data/audio_matrix/" 2>&1 | tail -1
...
hdr "Start ${TARGET} (ASR_DEVICE=${DEVICE}, vocal separation=${SEPARATION})"
...
extra_env_prefix=""
for setting in ${EXTRA_ENV+"${EXTRA_ENV[@]}"}; do
	extra_env_prefix="${extra_env_prefix}${setting} "
	note "extra env: ${setting}"
done
ssh_run "cd ${WORK_DIR} && HOST_INTEL_RENDER_GID=${RENDER_GID} BUILD_TARGET=${TARGET} ASR_DEVICE=${DEVICE} \
  ASR_ENGINE=${ENGINE} ASR_PREPROCESS_DEVICE=${PREPROCESS} ENABLE_VOCAL_SEPARATION=${SEPARATION} \
  ${extra_env_prefix}docker compose ${COMPOSE_FILES} up -d" || die "remote start failed"
...
# silent CPU fallback.
# `|| true`, for the same reason the audit pipeline above carries one: grep exits 1 when it
# matches nothing, the --wsl path runs this under `set -o pipefail`, and the outer script's
# `set -e` then aborts the run here -- after the header, with no banner and no explanation,
# on a service that started perfectly well but logged a banner this pattern did not match.
ssh_run 'docker logs whisper-pro-asr 2>&1 | grep -E "Whisper Pro ASR [0-9]|ASR Engine  |ASR Runtime|Preprocess Device|Resource Pool|OpenVINO devices" | tail -8 || true'
...
accuracy)
	SUITE_ARGS="tests/integration/test_transcription_accuracy.py"
	SUITE_ENV=""
	;;
smoke)
	SUITE_ARGS='tests/real_audio -m "$PYTEST_MARK"'
	SUITE_ENV="-e PYTEST_MARK='real_audio and smoke'"
	;;
full)
	SUITE_ARGS="tests/real_audio -m real_audio"
	SUITE_ENV=""
	;;
...
stress)
	SUITE_ARGS='tests/real_audio -m real_audio --deselect tests/real_audio/test_longform_stress.py'
...
if [ "$SUITE" = "stress" ]; then
    ...
    whisper-pro-asr-test:latest /bin/bash -c 'set -o pipefail; python3 -m pytest tests/real_audio/test_longform_stress.py -ra --no-cov --tb=short -s 2>&1 | tail -40'" ||
		{
			note "long-form stress reported failures (see above)"
			SUITE_FAILED=true
		}
fi

# What the run *measured*, as opposed to what the startup banner predicted. These differ,
# and the difference is the whole point: the banner named the iGPU as the preprocess device
# on a host where /status then reported UVR=CPU, because the banner reports the request and
# /status reports what a real inference actually touched. Capturing only the banner is how a
# preprocessing claim passes validation while running on the CPU throughout. This has to run
# after the suite, because "measured" means some inference has already happened.
hdr "Measured execution (post-suite)"
ssh_run 'docker logs whisper-pro-asr 2>&1 | grep -iE "ONNX provider|Isolation complete" | tail -3 || true'
ssh_run 'curl -s --max-time 20 http://127.0.0.1:9000/status' |
	python3 "${REPO_ROOT}/scripts/status_units.py" ||
	note "could not read measured execution from /status"
...
note "target: $TARGET (ASR_DEVICE=$DEVICE, ASR_ENGINE=$ENGINE, preprocess=$PREPROCESS, separation=$SEPARATION)"
