...
#: SHA-256 of the Silero VAD model at the pinned tag below.
#:
#: Provenance, because a checksum nobody can re-derive is not an integrity check. Verified
#: 2026-09-07 by fetching
#: https://github.com/snakers4/silero-vad/raw/v5.1.2/src/silero_vad/data/silero_vad.onnx
#: (2327524 bytes), loading it as an ONNX Runtime session, and confirming the Silero v5
#: signature -- inputs input/state/sr, outputs output/stateN -- plus a real inference over
#: 512 silent samples returning a speech probability of 0.044.
#:
#: The previous value (1a153a22...) matched no artefact at this tag, so _download_silero_vad_direct
#: raised "checksum mismatch" on every fetch and _ensure_vad_model exited 1. It failed loudly
#: rather than silently, which is why it survived: this constant is only read by the optional
#: pre-baking path, never by runtime provisioning.
...
def _download_silero_vad_direct(target_file):
    ...
    # A tag, not master. The SHA-256 below pins the bytes, so a moved master fails loudly
    # rather than silently -- but it fails at provisioning time on every host at once, for a
    # change upstream made to a branch this project never chose to follow.
...
def _ensure_uvr_model():
    ...
    # PERSISTENT_TEMP_DIR unconditionally: ensure_uvr_model's second argument is temp_root,
    # a scratch directory it extracts through -- not a model cache. Passing --cache-dir here
    # pointed the separator's temp work at the download cache, which is the directory the
    # preloader then seeds and reads back.
    if not model_provisioning.ensure_uvr_model(UVR_DIR, config.PERSISTENT_TEMP_DIR):
        sys.exit(1)
...
if __name__ == "__main__":
    # Configured here rather than at import: this module is imported by tests and by other
    # tooling, and basicConfig at module scope reconfigured the root logger of whatever
    # imported it -- a side effect of an import that only the script itself should have.
    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s", stream=sys.stdout)
    # Recorded because the image's torch build decides which accelerators are reachable,
    # and a preload log without it cannot explain a later device surprise. Also keeps the
    # import load-bearing: it is deliberately made before modules.core pulls in the ONNX
    # Runtime bootstrap.
    logger.info("Preload starting with torch %s", torch.__version__)
