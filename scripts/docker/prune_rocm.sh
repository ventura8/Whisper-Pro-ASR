...
verify_no_unmet_deps() {
	local unmet failed=0 lib dir
	# Every directory prune_arch_files touched, which is ROCM_DIRS -- not just the two
	# hardcoded ROCm paths. The extra directories passed as arguments (torch's bundled
	# site-packages/torch/lib, ~6GB of the same per-architecture kernels) were pruned and
	# then never checked, so a library left with an unmet dependency there shipped silently
