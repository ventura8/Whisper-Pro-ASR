...
#
#   .\setup_windows_remote.ps1 -PublicKeyBase64 '<base64 of ssh-ed25519 AAAA... comment>'
#
# The key arrives base64-encoded so it is never spliced into PowerShell source, neither by
# the generated one-liner nor by the UAC relaunch below: an unusual comment field containing
# a quote would otherwise break out of the string literal it is pasted into.
...
# Decoded once, into data. Every later use reads $PublicKey; the encoded form is what gets
# passed onward, so the raw key is never re-embedded in a command line.
...
if (-not $isAdmin -and -not $NoElevate) {
    ...
    try {
        $argLine = '-NoProfile -ExecutionPolicy Bypass -NoExit -File "' + $self +
                   '" -PublicKeyBase64 "' + $PublicKeyBase64 + '" -NoElevate'
        # -Wait is load-bearing. The generated one-liner writes this script to $env:TEMP and
        # removes it in a `finally` as soon as THIS process exits; without waiting, that
        # cleanup fires while the elevated window is still starting, and the elevated
        # PowerShell dies on a -File path that no longer exists. Waiting also means the
        # operator sees this window finish only once setup has actually completed.
        Start-Process -FilePath 'powershell' -Verb RunAs -ArgumentList $argLine -Wait
        Write-Host ""
        Write-Host "Setup ran in the elevated window. Read the READY line there." -ForegroundColor Magenta
        exit 0
    } catch {
...
if (-not $distros -and $isAdmin) {
    ...
    # $ErrorActionPreference is 'Stop' for this script, and wsl.exe writes progress to
    # stderr even on success -- which PowerShell turns into a terminating error, aborting the
    # bootstrapper at the exact step meant to recover from a missing distro. The same guard
    # the sibling setup scripts use around their ssh probes.
    $prior = $ErrorActionPreference
    try { $ErrorActionPreference = 'Continue'; wsl --install -d Ubuntu --no-launch } finally { $ErrorActionPreference = $prior }
...
if (-not $distros) {
    ...
    $distro = $distros[0]
    # Same guard: a distro with no Docker integration writes to stderr, and under 'Stop'
    # that aborted the script instead of reaching the warning below that says how to fix it.
    $prior = $ErrorActionPreference
    try {
...
Write-Host ""
Write-Host "READY user=$($env:USERNAME) admin=$isAdmin keyfile=$keyFile docker=$dockerVersion distro=$distro" -ForegroundColor Magenta
