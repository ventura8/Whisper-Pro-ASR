...
def build_speech_after_silence(dest: Path, params: dict, context: dict) -> None:
    ...
    # The name is fixed, so a lead file left behind by a failed concat is picked up by the
    # next entry that uses this builder -- and it is 30 seconds of silence, so the artifact
