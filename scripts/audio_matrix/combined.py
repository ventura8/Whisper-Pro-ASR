...
def _render_leg(leg: dict, index: int, context: dict) -> Path:
    ...
    # A completion flag and one finally, rather than an `except BaseException` that re-raises:
    # the flag covers cancellation and KeyboardInterrupt for free, which is the only reason
    # the handler was that broad, and there is now a single cleanup path to reason about.
    #
    # A half-written leg is worse than none: build()'s cleanup only removes legs it was
    # handed, and an incomplete one never gets returned -- so a partial dest would survive
    # and be concatenated by the next attempt.
    complete = False
    try:
...
def build(entry: dict, dest: Path, context: dict) -> None:
    ...
    try:
        ...
        # Named by appending, not with_suffix: `dest` here is the caller's staging path
        # ("<id>.partial.wav"), and with_suffix(".legs.json") would have produced the FINAL
        # "<id>.legs.json" -- publishing the sidecar before the audio it describes exists,
        # and leaving it behind describing the previous render if the publish then failed.
        # cli._render_atomically moves this into place only after the audio is published.
