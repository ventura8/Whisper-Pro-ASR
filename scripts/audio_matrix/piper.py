...
def ensure_voice(cache: Path, voice: str, expected_md5: str) -> Path:
    ...
    # Both halves. Piper reads <voice>.onnx.json for the phoneme map and sample rate, so a
    # cache holding only the .onnx satisfied this check and then failed inside synthesis --
    # once per clip, with an error naming neither the voice nor the missing sidecar.
    if not (model.exists() and model.with_suffix(".onnx.json").exists()):
