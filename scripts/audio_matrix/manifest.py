...
def _section_errors(section: str, entries: list[dict]) -> list[str]:
    ...
    Only ``clips`` gets the full check set: a combined entry carries ``legs`` rather than one
    ...
    for entry in entries:
        ...
        if not isinstance(entry, dict):
            # A bare string or number where an object belongs is an ordinary hand-edit slip,
            # and `entry.get` raised AttributeError from inside validation -- the one place
            # whose entire job is to report a malformed manifest legibly.
            messages.append(f"{section}: expected an object, got {type(entry).__name__} {entry!r}")
            continue
        messages.extend(_entry_messages(section, entry, checks))
    return messages
...
def _identified_entries(data: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    for section, entries in _all_sections(data):
        # isinstance before .get, as in _section_errors: a bare string where an object
        # belongs raised AttributeError here and aborted validate() -- so the readable
        # "expected an object, got str" message that _section_errors had already produced
        # never reached the caller.
        pairs.extend((section, entry["id"]) for entry in entries if isinstance(entry, dict) and entry.get("id"))
    return pairs
...
def validate(data: dict[str, Any]) -> list[str]:
    ...
    ``longform`` is checked alongside the list sections. It is generated and cached exactly
    like them, so a longform entry with no id is the same defect -- but it was reaching the
    generator, where `spec['id']` raised a bare KeyError from inside the build instead of
    being reported here with the rest of the manifest's problems.
    """
    errors = [message for section, entries in _all_sections(data) for message in _section_errors(section, entries)]
