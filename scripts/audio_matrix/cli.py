...
def _longform_digest(spec: dict, sources: list[dict], context: dict) -> str:
    ...
    stress grid for a byte-identical file. ``known_defects`` and ``translate`` are dropped
    for the same reason: recording a defect, deleting one when it is fixed, or opting a
    variant into the translation assertions changes what the tests do with the clip and
    nothing about the clip, and each used to cost a 20-minute rebuild of the same bytes.
    """
    entry = _without(spec, ("variants", "known_defects", "translate"))
