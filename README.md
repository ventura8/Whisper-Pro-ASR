...
- **Cooperative Pre-emption**: High-priority operations (such as language detection) pause long-running ASR at deterministic checkpoints, including pre-vocal-separation, HQ-prep FFmpeg progress boundaries, and pre-inference, ensuring responsive API behavior under saturation. A paused task holds no worker channel: a decode that is asked to pause closes its stream and resumes on what is left, so the priority task's own inference is never waiting on the task it paused.
...
| `ASR_SEGMENT_FIRST` | `true` | Detect the language of each speech region before decoding, group regions into runs of one language, and decode each language in its own call, one clip per region, instead of in fixed 30-second blocks. FASTER-WHISPER only; applies to auto-detected requests and to every translation (its output is English whatever was spoken, so a named language there is the main audio, not a constraint), and not to a transcription in a language the caller named. A decode window is the unit of language commitment, so this is what lets the language follow a switch faster than 30s. Measured on the 20-minute long-form fixture: wrong-language windows 67/118 -> **0/118**, hallucinated text in silence 11/25 -> 0/25. The cost depends on the device: decoding the fixture directly, clips took RTF 0.044 to 0.086 (~1.95x) on an RTX 3080; through the service the same card measured 1.5x, an RTX 5090 was *faster* (silence is never decoded), and a CPU pays ~2.3x. |
| `ASR_SEGMENT_LANGUAGES` | `true` | Report a language per segment rather than only per file. The per-region detection behind it always runs with `ASR_SEGMENT_FIRST` -- it is what groups regions into runs of one language -- so this switch changes only what the response reports, not the cost. |
| `ASR_FORCE_TRANSCRIPTION` | `false` | Force transcription: one subtitle in the file's language for a film that switches. Runs in the file's language (the one you named, or the detected one) are transcribed; every run in another language is **translated into English** -- the only target Whisper translates to -- so a Romanian film with Turkish scenes comes back as Romanian dialogue plus English for the Turkish, instead of the Turkish decoded under the Romanian token into noise. Off by default because it changes what a plain transcription returns; a transcription that names its language follows the switches when this is on. Per request: `force_transcription=true` (or `false`, to override a deployment default). |
| `SEGMENT_SPLIT_MIN_SILENCE_MS` | `250` | Silence needed to end a speech region during the clip scan. Separate from the decode VAD: this decides where the language may change, not what gets transcribed. |
| `SEGMENT_SPLIT_PAD_MS` | `200` | Padding added to each side of a region, so a clip boundary never clips a word. |
| `SEGMENT_FIRST_MIN_REGIONS` | `2` | Regions a file needs before clipping is worth it. One region commits to one language whatever is done to it. |
| `SEGMENT_RUN_MIN_SWITCH_SEC` | `3.0` | Speech a block in another language must hold, confidently detected, before it becomes its own decode run. Language detection on a single ~1s line is wrong about one time in five on real film; a run of several seconds is not. Shorter blocks are absorbed into the run around them. Returning to the language that holds most of the file needs no evidence. Each language is then decoded in one call, told its language outright, one clip per speech region. |
| `SEGMENT_RUN_MIN_SWITCH_SHARE` | `0.2` | On a short clip the bar above is the lesser of the seconds and this share of the labelled speech: half of a five-second file is not a slip, and the six code-switched fixtures are exactly that shape. Only reaches files with under 15 s of speech. |
| `SEGMENT_CLIP_MERGE_GAP_SEC` | `0.3` | Rejoin regions split by a pause shorter than this. Silero splits at breaths inside an utterance, and those sub-second fragments are where language detection is least reliable. Raising it past ~0.4s starts merging genuinely different languages into one clip. |
