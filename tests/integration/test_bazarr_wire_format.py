...
class TestRealBazarrWireFormat:
    ...
    def test_asr_video_file_param_present_takes_priority_over_uploaded_audio(
        ...
        distinct from the uploaded audio, then asserts convert_to_wav is called with
        the video_file path (not the uploaded audio) and that stream alignment was
        probed against it. Also asserts the request's encode=false raw-PCM input_flags
        (meant for the now-bypassed upload) are NOT applied to the video_file container
        -- that would make FFmpeg misinterpret a real media container as headerless PCM."""
        ...
        materialized_upload: dict[str, str] = {}
        original_resolve_upload = routes_utils.resolve_and_materialize_upload

        async def capture_materialized_upload(*args, **kwargs):
            result = await original_resolve_upload(*args, **kwargs)
            materialized_upload["path"] = str(result[1])
            return result
        ...
        with (
            ...
            mock.patch("modules.api.routes.asr.routes_utils.resolve_and_materialize_upload", side_effect=capture_materialized_upload),
        ):
        ...
        assert not Path(materialized_upload["path"]).exists()
