...
class TestDetectLanguageEndpoint:
    ...
    def test_run_leader_detection_handles_internal_exception(self, tmp_path):
        ...
        resolved_local_path = str(media_path)
        shared_future = concurrent.futures.Future()
        dedupe_key = detect_coalescing.build_dedupe_key(resolved_local_path, None)
        inflight_detect_by_path = detect_coalescing.__dict__["_INFLIGHT_DETECT_BY_PATH"]
