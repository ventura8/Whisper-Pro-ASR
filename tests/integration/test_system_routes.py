...
from modules.api.routes import system
...
def test_update_settings_accepts_retention_values_at_ui_slider_cap(client: FlaskCompatibleClient):
...
@pytest.mark.parametrize(
    "setting",
    [
        ("TELEMETRY_RETENTION_HOURS", "0", 24, 1, 720),
        ("TELEMETRY_RETENTION_HOURS", "721", 24, 1, 720),
        ("LOG_RETENTION_DAYS", "0", 7, 1, 90),
        ("LOG_RETENTION_DAYS", "91", 7, 1, 90),
    ],
)
def test_parse_int_setting_uses_fallback_for_out_of_range_values(setting, monkeypatch):
    """Dashboard settings never expose invalid environment retention values."""
    env_key, raw_value, fallback, minimum, maximum = setting
    monkeypatch.setenv(env_key, raw_value)
    parse_int_setting = getattr(system, "_parse_int_setting")
    assert parse_int_setting(env_key, fallback, minimum, maximum) == fallback
