...
pytestmark = pytest.mark.skipif(
    ...
    reason="real ffmpeg binary not available",
)
...
def test_real_probe_streams_and_packets_reads_actual_language_tags(tmp_path):
    ...
    probe_streams_and_packets = getattr(utils_helpers, "_probe_streams_and_packets")

    result = probe_streams_and_packets(media_path)
...
def test_real_build_stream_alignment_directives_detects_delay_from_itsoffset(tmp_path):
    ...
    assert delay_filter == "adelay=500:all=1"
