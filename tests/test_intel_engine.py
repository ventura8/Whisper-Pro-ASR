...
def reset_mocks():
    ...
    # Explicitly clear side_effect on the new mock's children if needed
    # but since it's a new MagicMock(), it should be clean.
    yield
...
class TestIntelWhisperEngine:
    ...
    def test_transcribe_fallback_text_from_segments(self):
        """Test text generation from segments if main text is empty."""
        engine = intel_engine.IntelWhisperEngine("/path/to/model")

        mock_segment = mock.MagicMock()
        mock_segment.text = "Hello"
        mock_segment.start_ts = 0.0
        mock_segment.end_ts = 1.0
    ...
    def test_transcribe_result_parsing(self):
        ...
        assert res['segments'][0]['text'] == " Chunk"
        assert res['segments'][0]['start'] == 0.5
