...
def _category_for(report: pytest.TestReport) -> str:
    ...
    if hasattr(report, "wasxfail"):
        if report.outcome == "skipped":
            return "xfailed"
        # Only a genuine pass is an xpass. Treating every non-skipped wasxfail report as one
        # meant a report that carried wasxfail *and* failed was announced as an unexpected
        # pass -- a real failure rendered as a curiosity, in the one place a reader looks to
        # see whether the run is healthy.
        return "xpassed" if report.outcome == "passed" else report.outcome
