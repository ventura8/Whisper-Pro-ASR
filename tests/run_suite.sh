...
run_radon_complexity_gate() {
	...
	local source_list
	source_list="$(mktemp)"
	find modules scripts whisper_pro_asr.py tests/check_coverage.py -type f -name '*.py' \
		-not -path './.venv/*' \
		-not -path './node_modules/*' \
		-not -path './coverage-js/*' \
		-print0 >"$source_list"
	# Single radon invocation: -s shows score letters, output goes to the report
	# artifact and is also scanned for rank-B+ violations (letters B through F).
...
if stage_active python-tests; then
	...
	# Prepend a pytest-cov-style header so CI's MishaKav/pytest-coverage-comment
	# parser accepts the file (plain `coverage report` omits the `coverage:` line).
	set +e
	{
		echo "----------- coverage: platform linux, python $(python3 -c 'import sys; print("%d.%d.%d" % sys.version_info[:3])') -----------"
