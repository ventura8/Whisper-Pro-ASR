...
if [ "$SKIP_LINT" != "1" ]; then
  ...
  python3 -m autopep8 --in-place --recursive --max-line-length 140 modules tests whisper_pro_asr.py scripts

  echo "--- Running Pylint ---"
  python3 -m pylint --rcfile=.pylintrc modules tests whisper_pro_asr.py --recursive=y
