...
def _isolate_from_legacy_history() -> bool:
    ...
    remove it if so. An existing empty directory is reused (not deleted on exit).
    An existing non-empty directory is rejected so E2E never reads/writes leftover
    history/telemetry from a prior run or a real state tree.
    """
    state_dir = os.environ["WHISPER_STATE_DIR"]
    created_state_dir = not os.path.isdir(state_dir)
    _reject_nonempty_state_dir(state_dir, created_state_dir)
    os.makedirs(state_dir, exist_ok=True)
    _import_in_isolated_state(state_dir, created_state_dir)
    return created_state_dir


def _reject_nonempty_state_dir(state_dir: str, created: bool) -> None:
    if not created and os.listdir(state_dir):
        raise SystemExit(f"WHISPER_STATE_DIR {state_dir!r} exists and is not empty -- refusing to reuse it for E2E isolation.")


def _import_in_isolated_state(state_dir: str, created_state_dir: bool) -> None:
...
def main() -> None:
    ...
    try:
        ...
        # while an existing empty WHISPER_STATE_DIR is reused and never deleted
        # (non-empty dirs are rejected at startup in _isolate_from_legacy_history).
        if created_state_dir:
            shutil.rmtree(state_dir, ignore_errors=True)
