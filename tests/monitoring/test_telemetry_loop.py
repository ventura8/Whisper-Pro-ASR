...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Downloading outranks an active task. The gate holds tasks while weights arrive, so
        # a task can be active and stage-matched while the engine is not loaded at all --
        # reporting "busy" there told the dashboard the model was working when nothing was.
        ([{"status": "active", "stage": "Transcribing"}], True, False, "downloading"),
        ([{"status": "active", "stage": "Language Detection"}], True, True, "downloading"),
    ...
    ids=[
        "provisioning",
        "provisioning-beats-active-task",
        "provisioning-beats-active-detect",
        "transcription",
        "inference",
        "translation",
        "inactive",
