...
def test_unsupported_language_code_is_handled(spanish_clip):
    ...
    # Accepting the request is only half the contract. A 200 carrying `{"text": ""}` is the
    # silently-empty result this module's docstring rules out, and checking only the status
    # code let an unknown code degrade into exactly that without failing anything.
    if response.status_code == 200:
        _assert_accepted_transcript(response, "an unsupported language code")
