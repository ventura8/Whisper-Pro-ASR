...
def _sentences(text: str) -> list[str]:
    ...
    Short units are dropped unless the segment is nothing but copies of one, or holds a
    run of one longer than any speech contains: "yes" in a sentence is speech, "yes" as
    the only content of a segment eight times over is not, and neither is "yes" eight
    times over followed by a sentence -- a loop the decoder happened to recover from.
    """
    units: list[str] = []
    ...
    if len(units) > 1 and (len(set(units)) == 1 or _longest_run(units)[1] > MAX_SENTENCE_REPEATS):
        return units
