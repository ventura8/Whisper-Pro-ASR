...
#: clip id -> why generating it failed, for the life of the session. The generator is a
#: subprocess bounded at GENERATE_TIMEOUT_SEC, and a clip is requested by several tests, so
#: a missing phonemizer cost that timeout once per test rather than once per clip -- minutes
#: of dead wall clock to re-derive an answer that cannot change mid-session.
_GENERATION_FAILURES: dict[str, str] = {}
...
def resolve_clip(clip_id: str) -> Path:
    ...
    for candidate in (_committed_path(clip_id), _cached_path(clip_id)):
        if candidate.exists():
            return candidate
    # Checked after the paths above, never before: a clip that failed to generate can still
    # appear later in the session, because another test's generator run produces more than
    # the clip it asked for.
    if clip_id in _GENERATION_FAILURES:
        reason = _GENERATION_FAILURES[clip_id]
    else:
        reason = _generate(clip_id)
        _GENERATION_FAILURES[clip_id] = reason
