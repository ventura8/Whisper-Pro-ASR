...
class TestDetectionPipeline:
    ...
    def test_run_detection_pipeline_success(self):
        """Detection pipeline returns language and segment count on success."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        with mock.patch(
            "modules.language_detection._collect_active_speech_segments"
        ) as mock_collect:
    ...
    def test_run_detection_pipeline_no_segments(self):
        """Detection pipeline handles no speech segments."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        with mock.patch(
            "modules.language_detection._collect_active_speech_segments"
    ...
    def test_run_detection_pipeline_err_fallback(self):
        ...
        model = _make_ld_model()
        processor = _make_ld_processor()
        mock_path = "modules.language_detection._collect_active_speech_segments"
        with mock.patch(mock_path, side_effect=ValueError("fail")):
            with mock.patch(
                "modules.language_detection._sample_audio_segments"
            ) as mock_sample:
                mock_sample.return_value = [np.zeros(16000)]
                mock_detect = (
                    "modules.language_detection._detect_languages_from_segments"
                )
...
class TestLanguageDetectionHelpers:
    ...
    def test_detect_languages_fallback(self):
        """Fallback detection returns per-segment language probs."""
        model = _make_ld_model()
        processor = _make_ld_processor()
...
class TestLanguageDetectionSegments:
    """Tests for read_segment, collect, search, sample, detect_from_segments."""

    def test_read_segment_mono_resample_pad(self):
        """Read segment resamples and pads to target."""
        with mock.patch("modules.language_detection.sf.info") as mock_info:
            ...
            with mock.patch(
                ...
                segment = language_detection._read_segment(
                    "dummy.wav", 0, 1, 44100
                )
                assert segment.shape == (480000,)

    def test_collect_active_speech_segments(self):
        """Collect segments returns expected count."""
        with mock.patch("modules.language_detection.sf.info") as mock_info:
            mock_info.return_value.duration = 100
            mock_info.return_value.samplerate = 16000
            with mock.patch(
                "modules.language_detection._read_segment"
            ) as mock_read:
                mock_read.return_value = np.zeros(480000)
                segments = language_detection._collect_active_speech_segments(
                    "dummy.wav"
                )
    ...
    def test_sample_audio_segments(self):
        """Sample audio segments respects duration and rate."""
        with mock.patch("modules.language_detection.sf.info") as mock_info:
            ...
            with mock.patch(
                ...
                mock_read.return_value = np.zeros(480000)
                segments = language_detection._sample_audio_segments("dummy.wav")
                assert len(segments) == 5

    def test_detect_languages_from_segments_no_mapping(self):
        """Detect from segments uses fallback when no token mapping."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        ...
        with mapping_patch:
            with mock.patch(
                ...
                segments = [np.zeros(16000)]
                res = language_detection._detect_languages_from_segments(
                    model, processor, segments,
                )
                assert res == [{"en": 1.0}]
...
class TestDetectSingleSegmentProbs:
    """Tests for _detect_single_segment_probs."""

    def test_detect_single_segment_probs_cpu_friendly(self):
        """Single-segment probs return aggregated language dict."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        lang_ids = [1, 2]
        ...
        with mock.patch(
            ...
            res = language_detection._detect_single_segment_probs(
                model, processor, segment, lang_ids, id_to_lang
            )
            assert res == {"en": 0.99, "fr": 0.01}

    def test_detect_single_segment_probs_no_valid_ids(self):
        """Single-segment probs handle invalid lang ids."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        lang_ids = [100, 200]
    ...
    def test_detect_single_segment_probs_non_list_softmax(self):
        """Single-segment probs handle scalar softmax result."""
        model = _make_ld_model()
        processor = _make_ld_processor()
        lang_ids = [1]
        id_to_lang = {1: "en"}
