...
def mock_fw_vad(monkeypatch):
    ...
    monkeypatch.setattr(vad, 'fw_get_ts', mock_get_ts)
    monkeypatch.setattr(vad, 'fw_decode', mock_decode)
    monkeypatch.setattr(vad, 'fw_Opts', mock_options)
...
def test_decode_audio_missing_backend(monkeypatch):
    ...
    monkeypatch.setattr(vad, 'fw_decode', None)
