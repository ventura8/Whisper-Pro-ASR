...
class TestAmdProviderConfig:
    ...
    def test_cpu_fallback_when_no_amd_provider_is_available(self):
        """Falls back to CPU execution provider when no AMD provider can be selected."""
    ...
    def test_dml_selected_when_device_node_present(self):
        """DmlExecutionProvider is selected on Windows when DirectML is available."""
        with (
            mock.patch("modules.inference.pipeline.openvino_provider_dispatch.sys.platform", "win32"),
    ...
    def test_dml_ignored_on_linux_even_if_listed(self):
        """DirectML is Windows-only and must not be selected in Linux containers."""
        with (
            mock.patch("modules.inference.pipeline.openvino_provider_dispatch.sys.platform", "linux"),
    ...
    def test_resolve_amd_fallback_message_branches(self):
        ...
        # Branch 1: WSL2 without /dev/kfd falls back to CPU in Linux containers
        wsl_msg = resolve_msg_fn(False, True, False)
        assert "cannot use DirectML" in wsl_msg
...
class TestAutoProviderConfigAmd:
    ...
    def test_auto_falls_back_to_cpu_when_no_cuda_or_amd_provider_exists(self):
        """Auto selection falls back to CPU when CUDA and AMD providers are absent."""
...
class TestPreprocessingProviderAmd:
    ...
    def test_amd_device_type_falls_back_to_cpu_without_amd_provider(self):
        """AMD preprocessing falls back to CPU execution provider when no AMD provider is available."""
...
class TestAppendAmdUnits:
    ...
    def test_wsl_dxg_without_kfd_registers_nothing(self):
        """WSL /dev/dxg without /dev/kfd must not register a schedulable AMD unit."""
        append_amd_units_fn = getattr(ch, "_append_amd_units")
        units: list = []
        with (
            mock.patch("modules.core.config_helpers._count_amd_drm_devices", return_value=0),
            mock.patch("os.path.exists", return_value=False),
            mock.patch("modules.core.config_helpers._has_amd_win32_hardware", return_value=False),
            mock.patch("modules.core.config_helpers._has_amd_wsl_hardware", return_value=True),
        ):
            append_amd_units_fn(1, units)
        assert not units

    def test_win32_directml_registers_one_unit(self):
        """Windows DirectML hosts register a single AMD unit when DRM/KFD are absent."""
        append_amd_units_fn = getattr(ch, "_append_amd_units")
        units: list = []
        with (
            mock.patch("modules.core.config_helpers._count_amd_drm_devices", return_value=0),
            ...
            append_amd_units_fn(1, units)
        assert units == [{"type": "AMD", "id": "amd:0", "name": "AMD GPU 0"}]
