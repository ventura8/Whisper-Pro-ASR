...
def reset_client_state() -> Generator[None, None, None]:
    ...
    Also clears _SHUTDOWN_REQUESTED: it's a module-level Event set by shutdown()
    and never cleared on its own success path outside the lock-protected block --
    a test that exercises shutdown() without going through the normal call()/
    shutdown() pairing could otherwise leak it set into a later test, making that
    test's call() raise WhisperXWorkerError spuriously."""
    _STATE["process"] = None
    _STATE["conn"] = None
    _STATE["generation"] = 0
...
def test_shutdown_interrupts_a_hung_unbounded_call_instead_of_deadlocking():
    ...
    assert (len(call_errors), "shut down" in str(call_errors[0]), _STATE["process"], _STATE["conn"]) == (1, True, None, None)
