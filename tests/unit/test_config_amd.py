...
class TestConfigAmd:
    ...
    def _reset_config_module(self) -> None:
        with (
            mock.patch.dict(os.environ, self._build_baseline_env({}), clear=True),
    ...
    def test_detect_explicit_amd_first_with_cuda(self):
        """Test _detect_explicit_amd_first preserves explicit AMD choice when CUDA is present."""
        env = self._build_baseline_env({"ASR_DEVICE": "AMD", "ASR_PREPROCESS_DEVICE": "AUTO", "MAX_AMD_UNITS": "1"})
        try:
            with (
                mock.patch.dict(os.environ, env, clear=True),
...
class TestAmdHardwareDetectionHelpers:
    """Unit tests for AMD detection helpers used by config loading."""
    ...
    def test_has_amd_wsl_driver_folder_nested(self, tmp_path: Path):
        """Adrenalin WSL drivers may nest amdxc64.so one directory below the INF folder."""
        nested = tmp_path / "u0419539.inf_amd64" / "B419298"
        nested.mkdir(parents=True)
        (nested / "amdxc64.so").write_bytes(b"")
        folder_fn = getattr(config_helpers_module, "_has_amd_wsl_driver_folder")
        assert folder_fn(str(tmp_path / "u0419539.inf_amd64")) is True
        assert folder_fn(str(tmp_path / "missing.inf_amd64")) is False
    ...
    def test_detect_amd_hardware_exception(self):
        ...
        with mock.patch("modules.core.config_helpers._has_amd_hardware", side_effect=RuntimeError("boom")):
            detect_amd(1, hw_units, state)
        assert not hw_units
        assert state == {"device": "CPU", "prep_device": "CPU", "compute": "int8"}
