...
"""
    ...
    def test_get_custom_mount_points_decodes_octal_escapes(self):
        ...
        Driven through ``config_module.get_custom_mount_points()`` rather than by calling
        ``mount_helpers._extract_mount_point`` directly: the decoding only matters because
        the decoded value reaches APPROVED_ROOTS, and a private-helper test would still pass
        if the parser stopped calling it. \\040 is the octal escape for a space (0x20).
        """
        fake_mounts = "/dev/sdb1 /my\\040movies ext4 rw,relatime 0 0\n"
        with (
            mock.patch("os.path.exists", return_value=True),
            mock.patch("builtins.open", mock.mock_open(read_data=fake_mounts)),
        ):
            assert "/my movies" in config_module.get_custom_mount_points()
    ...
    def test_persistent_temp_dir_file_exists_fallback(self):
        ...
        # The one path this test is about: the persistent temp directory config derives
        # from OV_CACHE_DIR. `"temp" in str(path)` matched anything with those four letters
        # anywhere in it -- a pytest tmp_path, /tmp, a "templates" directory -- so the stub
        # reached well outside the branch under test and what it broke depended on which
        # other directories config happened to create on the machine running it.
        persistent_temp = os.path.normpath(os.path.abspath(os.path.join(config_module.OV_CACHE_DIR, "temp")))
        ...
        def mock_makedirs(path, *_args, **_kwargs):
            ...
            if os.path.normpath(os.path.abspath(str(path))) == persistent_temp:
