...
class TestTheBaseDigestIgnoresTheVariantList:
    ...
    def test_recording_a_defect_or_opting_into_translation_does_not_stale_the_clip(self, context):
        """Both change what the tests do with the clip and nothing about the clip: a fix is
        recorded by deleting a defect, and that must not cost a 20-minute rebuild."""
        base = {"id": "longform_film", "target_seconds": 1200, "languages": ["en", "ru"], "profile": "film"}
        with mock.patch.object(cli, "_tool_versions", return_value={"piper": "1"}):
            plain = cli._longform_digest(base, [], {"defaults": {}})
            annotated = cli._longform_digest({**base, "known_defects": {"quiet": "measured"}, "translate": True}, [], {"defaults": {}})
