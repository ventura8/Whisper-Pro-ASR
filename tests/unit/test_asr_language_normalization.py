...
def test_a_posix_style_locale_resolves_like_its_hyphenated_twin():
    """`en_US` and `en-US` name the same language and must not diverge.

    The recorded defect: only "-" was split on, so the POSIX spelling reached the ISO 639-2
    lookup whole, matched nothing and silently auto-detected -- while the identical
    hyphenated form resolved to "en". Nothing in the response said which had happened.
    """
    assert asr._normalize_language("en_US") == asr._normalize_language("en-US") == "en"
    assert asr._normalize_language("pt_BR") == "pt"
