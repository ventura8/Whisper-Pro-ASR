...
def test_vad_preload_download_failure_handling(tmp_path: Path):
    ...
    def validator(p: Path) -> bool:
        # A deliberately synthetic digest: this case is about a download that produced the
        # wrong bytes, so any value the payload cannot match will do. It used to carry what
        # looked like the real Silero pin, which is a trap -- a reader grepping for that
        # constant finds it here and takes it for the authoritative one.
        return model_integrity.verify_onnx_model_file(p, min_bytes=500 * 1024, expected_sha256="0" * 64)
