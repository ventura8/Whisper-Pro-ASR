...
class TestNaturalGapDistribution:
...
class TestNaturalLanguageLayout:
...
class TestStressProfileIsUnchanged:
    ...
    def test_stress_layout_switches_far_more_often_than_natural(self):
        ...
        sources = _sources()
        stress_rng = random.Random(longform.LAYOUT_SEED)
        stress = _speech(longform._plan(sources, stress_rng))
