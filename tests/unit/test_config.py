...
class TestConfigEnv:
    ...
    def test_app_constants(self):
        ...
        assert config_module.VERSION == "1.4.0"
...
class TestTheNpuCannotExecuteFallback:
    """What a failed NPU executability probe does to the device claim and the pool.

    The NPU builds a WhisperPipeline in seconds and only fails at inference, so without
    this path the service starts healthy, prints "ASR Runtime: OpenVINO (NPU)", and returns
    500 for every request. The probe runs at config import precisely because the scheduler
    and the isolated workers read HARDWARE_UNITS when their own modules import -- a later
    correction leaves the banner saying one thing while tasks are dispatched to another.
    """

    def _reload_with_npu(self, devices: tuple[str, ...]):
        """Reload config on an Intel host whose NPU is pinned as unable to execute."""
        env = {"ASR_DEVICE": "NPU", "ASR_ENGINE": "INTEL-WHISPER"}
        with mock.patch.dict(os.environ, env):
            with mock.patch("ctranslate2.get_cuda_device_count", return_value=0):
                mock_core = mock.MagicMock()
                mock_core.available_devices = list(devices)
                mock_core.get_property.return_value = "Intel(R) AI Boost"
                with mock.patch("openvino.Core", return_value=mock_core), npu_cannot_execute():
                    importlib.reload(config_module)
                    # Captured inside the patch. config.update_env writes ASR_DEVICE into
                    # os.environ, and mock.patch.dict restores the original mapping on exit --
                    # so a caller asserting on os.environ afterwards was reading the restored
                    # value, not the one the reload produced.
                    return config_module, os.environ.get("ASR_DEVICE")

    def test_asking_for_the_npu_on_a_mixed_host_still_leaves_an_npu_only_pool(self):
        """ASR moves to the CPU, and the pool is already NPU-only by the time the probe runs.

        Worth stating explicitly, because config.py's fallback has two branches and only
        one of them is reachable this way. ASR_DEVICE=NPU makes
        _restrict_pool_to_requested_device narrow the pool to the NPU *before* the probe --
        so the "remove the NPU, keep the other units" branch has nothing to keep, and the
        documented single-unit behaviour applies instead: the unit stays, typed NPU, while
        the ASR device claim becomes CPU. IntelWhisperEngine's own warmup then moves that
        engine to the CPU itself, which is the path validated on the NUC.
        """
        reloaded, asr_device_env = self._reload_with_npu(("GPU.0", "NPU.0", "CPU"))

        assert reloaded.DEVICE == "CPU"
        assert reloaded.ASR_DEVICE_ENV == "CPU"
        assert asr_device_env == "CPU", "workers inherit this and skip detection"
        assert [u["type"] for u in reloaded.HARDWARE_UNITS] == ["NPU"], "the explicit device request narrowed the pool first"

    def test_an_npu_only_pool_keeps_the_unit(self):
        """Emptying the pool would leave the scheduler unable to serve at all.

        The unit stays, and stays typed NPU. Retyping it to CPU was tried and reverted:
        engine_factory routes by unit type, so a CPU-typed unit degraded INTEL-WHISPER to
        Faster-Whisper and handed the OpenVINO IR to CTranslate2 -- whose corruption
        handler can then delete the OpenVINO weights.
        """
        reloaded, _asr_device_env = self._reload_with_npu(("NPU.0", "CPU"))

        assert reloaded.DEVICE == "CPU"
        assert reloaded.ASR_DEVICE_ENV == "CPU"
        assert [u["type"] for u in reloaded.HARDWARE_UNITS] == ["NPU"]

    def test_the_igpu_survives_in_the_list_preprocessing_reads(self):
        """DETECTED_UNITS is taken before the engine filter, so UVR keeps both devices.

        This is what makes the fallback tolerable: ASR gives up on the NPU, but neither the
        NPU nor the iGPU pruned by the ASR_DEVICE request is lost to vocal isolation, which
        runs on ONNX Runtime and reaches both.
        """
        reloaded, _asr_device_env = self._reload_with_npu(("GPU.0", "NPU.0", "CPU"))

        assert any(u["type"] == "NPU" for u in reloaded.DETECTED_UNITS), "preprocessing reads this list"
        assert any(u["type"] == "GPU" for u in reloaded.DETECTED_HARDWARE_UNITS), "detection still found the iGPU"
