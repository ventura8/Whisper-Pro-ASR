...
def test_prune_rocm_does_not_delete_rocsolver():
...
def test_prune_rocm_only_removes_architecture_named_files():
...
def test_verify_fails_on_a_missing_dependency(tmp_path: Path):
...
def test_verify_passes_when_every_dependency_resolves(tmp_path: Path):
...
def test_verify_ignores_runtime_injected_driver_and_tensorrt(tmp_path: Path):
