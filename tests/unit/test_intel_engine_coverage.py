...
def test_intel_engine_init_successful_retry_after_recovery(tmp_path):
    ...
    with (
        ...
        def fake_purge(target_path, description=""):
            # Recorded, not asserted here. An AssertionError raised inside this side effect is
            # caught by IntelWhisperEngine's own `except (RuntimeError, ValueError, ...)`
            # handling around the purge, so a wrong target_path would surface as a generic
            # init failure -- or be swallowed entirely -- instead of failing this test.
            purge_calls.append((target_path, description))
    ...
    # Asserted out here, on the main thread, where a failure cannot be caught by the engine.
    assert len(purge_calls) == 1, "the purge must run exactly once"
    purged_path, reason = purge_calls[0]
    assert purged_path == str(model_dir), "the purge must target the model directory"
    assert reason, "the purge must carry a description, which is what the operator sees in the log"
