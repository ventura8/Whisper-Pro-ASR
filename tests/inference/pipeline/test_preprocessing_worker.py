...
class _FakeManager:
    ...
    def preprocess_audio(self, audio_path, force=False, yield_cb=None, stage="Vocal Separation"):
        ...
        if self.mode == "blocked":
            self.started.set()
            assert self.release.wait(timeout=5), "test must release the fake manager"
            try:
                if yield_cb:
                    yield_cb()
...
def test_closing_separation_stream_aborts_the_manager_yield_callback(fake_manager, monkeypatch):
    ...
    class _ThreadingWithFakeEvent:
        """The real threading module with only Event swapped out.

        Listing the substitutes explicitly (Event plus Thread) silently removed every other
        attribute, so any later use of threading.Lock or threading.current_thread inside the
        worker would fail as a missing attribute rather than doing what it does in
        production. Delegating the rest keeps the substitution to the one name it is about.
        Patched on the module under test, never on the real threading module, which pytest
        and its own threads are using at the same time.
        """
    ...
    monkeypatch.setattr(preprocessing_worker, "threading", _ThreadingWithFakeEvent())
    monkeypatch.setattr(preprocessing_worker, "_HEARTBEAT_SEC", 0.001)
