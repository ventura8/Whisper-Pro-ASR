...
class TestDetectLanguageBatch:
    ...
    def test_the_decoded_audio_is_kept_between_chunks_of_the_same_file(self, monkeypatch):
        ...
        # A second unit's file arriving between the first's chunks must not evict it: the
        # worker serves every unit of its engine type, and the chunks interleave.
        list(inference_worker._detect_language_regions("u", "/a.wav", [{"start": 2.0, "end": 3.0}]))
        assert decodes == ["/a.wav", "/b.wav"], "each file decoded once, whatever the interleaving"
        assert list(inference_worker._REGION_AUDIO) == ["/a.wav", "/b.wav"]
        inference_worker._unload_all()
        assert not inference_worker._REGION_AUDIO, "an idle purge releases it"

    def test_the_cache_is_bounded_oldest_out(self, monkeypatch):
        """An abandoned request cannot pin memory: past the limit the oldest file goes."""

        def import_module(name):
            if name.endswith("vad"):
                return SimpleNamespace(decode_audio=lambda path: list(range(16000)))
            return SimpleNamespace(run_language_detection_core=lambda *a, **k: {"detected_language": "en", "confidence": 0.9})

        inference_worker._REGION_AUDIO.clear()
        inference_worker._ENGINES["u"] = FakeEngine()
        monkeypatch.setattr(inference_worker.importlib, "import_module", import_module)
        for index in range(inference_worker._REGION_AUDIO_LIMIT + 1):
            list(inference_worker._detect_language_regions("u", f"/{index}.wav", [{"start": 0.0, "end": 0.5}]))
        assert len(inference_worker._REGION_AUDIO) == inference_worker._REGION_AUDIO_LIMIT
        assert "/0.wav" not in inference_worker._REGION_AUDIO, "the oldest is the one evicted"
        inference_worker._REGION_AUDIO.clear()

    def test_the_parent_can_release_the_samples_once_a_file_is_done(self, monkeypatch):
        """Between a file's last chunk and the next request the samples are dead weight (a
        two-hour film is 460 MB as float32); the parent asks for them to be dropped and is
        told whether there was anything to drop."""

        def import_module(name):
            if name.endswith("vad"):
                return SimpleNamespace(decode_audio=lambda path: list(range(10 * 16000)))
            return SimpleNamespace(run_language_detection_core=lambda *a, **k: {"detected_language": "en", "confidence": 0.9})

        inference_worker._REGION_AUDIO.clear()
        inference_worker._ENGINES["u"] = FakeEngine()
        monkeypatch.setattr(inference_worker.importlib, "import_module", import_module)
        assert inference_worker._release_region_audio("/a.wav") is False, "nothing decoded yet"
        list(inference_worker._detect_language_regions("u", "/a.wav", [{"start": 0.0, "end": 1.0}]))
        list(inference_worker._detect_language_regions("u", "/b.wav", [{"start": 0.0, "end": 1.0}]))
        assert inference_worker._release_region_audio("/a.wav") is True
        assert list(inference_worker._REGION_AUDIO) == ["/b.wav"], "only the finished file is dropped"
        inference_worker._REGION_AUDIO.clear()
