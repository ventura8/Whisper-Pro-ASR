...
class TestRepetitionPenaltyIsScopedToGreedy:
    ...
    def test_cpu_beam_search_leaves_the_penalty_untouched(self):
        ...
        rejected = "setting repetition_penalty alongside beam search is rejected by OpenVINO GenAI"
        assert getattr(config, "repetition_penalty", 1.0) == 1.0, rejected
...
def _only_for_model_path(monkeypatch, model_path: str, *, listdir_result: list) -> None:
    """Fake os.path.isdir/os.listdir for ``model_path`` alone, delegating everywhere else.

    ``lambda _p: True`` replaced the answer for EVERY path in the process, and both are
    read by pytest, importlib and the tmp-path machinery while the test runs -- so the fake
    reached far outside the code under test and the blast radius depended on what else
    happened to look at the filesystem.
    """
    real_isdir, real_listdir = os.path.isdir, os.listdir
    monkeypatch.setattr(intel_engine.os.path, "isdir", lambda p: True if p == model_path else real_isdir(p))
    monkeypatch.setattr(intel_engine.os, "listdir", lambda p: listdir_result if p == model_path else real_listdir(p))
...
class TestIntelInitializationBranches:
    ...
    def test_a_corrupted_model_is_purged_reprovisioned_and_retried(self, monkeypatch):
        ...
        _only_for_model_path(monkeypatch, "/models/ov", listdir_result=[])
