...
def test_a_handler_error_mid_stream_raises_and_leaves_the_channel_usable(channel):
    ...
    Unconditional now. The previous form fell back to a plain ``call("boom")`` whenever the
    fixture worker had no streaming failure handler -- which it did not -- so this test
    never once reached the mid-stream path it is named for.
    """
    pid_before = channel.call("echo", value="x")["pid"]

    with pytest.raises(worker_channel.WorkerError, match="stream exploded"):
        list(channel.stream("boom_stream"))

    assert channel.call("echo", value="still here")["echo"] == "still here"
    # The worker reported the failure and left the pipe clean, so it must survive: treating
    # an error event as non-terminal sent it down the abandon path, which drains for 30
    # seconds and then kills it -- a full model reload as the price of one handler error.
    assert channel.call("echo", value="y")["pid"] == pid_before, "a reported error must not cost the worker"
