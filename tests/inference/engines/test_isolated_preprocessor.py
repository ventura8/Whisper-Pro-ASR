...
def test_preemption_still_crosses_the_process_boundary(prep):
    ...
    done, total = progress["chunks_done"], progress["chunks_total"]
    assert done < total, f"worker completed {done}/{total} chunks; separation was not cancelled"

    # Only then is the channel checked: it must also be reusable rather than stranded.
    # `["loaded"] in (True, False)` was true of every bool, so this passed for a channel in
