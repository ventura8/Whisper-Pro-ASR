...
def test_torch_device_uses_xpu_on_an_intel_gpu_unit():
    with _with_torch(xpu=True):
        assert engine_factory._resolve_torch_device({"type": "GPU"}) == "xpu"


def test_torch_device_sends_an_npu_unit_to_the_cpu_not_the_igpu():
    """torch has no NPU backend, so an NPU unit must not be handed the Intel GPU.

    It used to share the XPU probe with GPU units, which put an NPU-assigned task on the very
    device the GPU unit already serves: both units' ASR then serialised on one iGPU instead
    of proceeding in parallel. This is the same policy config.py applies when the NPU cannot
    execute the Whisper IR -- fall back to the CPU and leave the NPU free for the UVR
    preprocessing it is actually good at (docs/REMOTE_VALIDATION.md).
    """
    with _with_torch(xpu=True):
...
def test_faster_whisper_engine():
    ...
    with mock.patch("importlib.import_module", return_value=mock_faster_whisper):
        ...
        # Test detect_language. Decoded samples, not a path: faster-whisper's detect_language
        # does no decoding of its own, so the engine decodes a path before delegating and only
        # already-decoded audio reaches the model unchanged. Passing "dummy_audio" here used to
        # pass, and that is exactly the shape that returned HTTP 500 in production --
        # see tests/inference/engines/test_faster_whisper_detect_language.py.
        samples = [0.0, 0.25, 0.5]
        mock_model.detect_language.return_value = ("en", 0.95, [("en", 0.95), ("fr", 0.05)])
        res = engine.detect_language(samples)
        mock_model.detect_language.assert_called_once_with(samples)
...
def test_faster_whisper_engine_local_path_purges_without_retry(tmp_path):
    ...
    # The call count is what actually pins "not retried": without it the purge assertion
    # passes whether or not a second load ran, and that second load would hand a
    # just-deleted local path to the downloader as if it were a repository id.
    assert not corrupt_local_dir.exists()
    assert mock_faster_whisper.WhisperModel.call_count == 1, "a purged local path must not be retried as a repo id"
...
def test_faster_whisper_engine_recovery_failure_raises(tmp_path):
    ...
    valid_dir.mkdir()
    # touch + truncate, as in test_openvino_model_protection.py: verify_ct2_model_dir reads
    # st_size and never the bytes, so writing 11MB of "x" to every run of this test bought
    # nothing. A sparse file reports the same size and costs no disk.
    model_bin = valid_dir / "model.bin"
    model_bin.touch()
    os.truncate(model_bin, 11 * 1024 * 1024)
