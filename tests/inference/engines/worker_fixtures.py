...
def _boom_stream():
    """Emit one event, then fail -- the mid-stream error terminator.

    One event first, deliberately: a handler that raises before yielding anything never
    reaches the streaming path at all, so it would exercise the plain call error instead of
    the "error" event the parent's stream loop has to recognise as terminal.
    """
    yield {"event": "segment", "index": 0}
...
def worker_main(conn):
    worker_runtime.serve(
        ...
        stream_handlers={"count": _count, "boom_stream": _boom_stream},
    )
