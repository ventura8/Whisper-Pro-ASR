...
# The float16 -> int8 CPU coercion is covered once, by
# test_create_whisperx_engine_coercion at the end of this module -- which asserts the same
# thing plus the model id and the device. A second copy here patched the same config value
# for the same call and checked a subset of the same result.


def test_whisperx_engine(monkeypatch):
    ...
    with mock.patch("modules.inference.engines.whisperx_engine.worker") as mock_worker:
        ...
        # monkeypatch, not a hand-rolled save/restore: pytest undoes it even when the test
        # fails part-way, whereas a try/finally around only part of the body left the real
        # get_audio_duration replaced for every later test in the session if anything above
        # the try raised.
        monkeypatch.setattr(engine_factory.utils, "get_audio_duration", mock.MagicMock(return_value=10.0))
...
def test_whisperx_engine_reloads_model_handle_after_worker_restart(monkeypatch):
