...
def _function_ast(fn: Callable[..., Any]) -> ast.AST:
    ...
    Matches by exact defining line number (`fn.__code__.co_firstlineno`), not
    just by name via `ast.walk` -- name-only matching would silently return
    the WRONG node (and so the wrong reentrancy classification) whenever two
    functions/methods anywhere in the module share a name, e.g. a duplicate
    method name on two different classes or a redefined module-level function.
    Falls back to a name match only if no node's line number lines up exactly
    (defensive; should not happen for a real function object)."""
