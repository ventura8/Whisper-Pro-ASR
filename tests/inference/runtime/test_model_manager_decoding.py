...
def test_a_translation_follows_the_switches_even_when_a_language_was_named(task):
    ...
    # The task reaches every engine call folded to lower case: faster-whisper looks up the
    # `<|task|>` token exactly, and the Intel engine silently transcribes on any spelling
    # outside its mapping.
    assert calls == [("ro", "translate", [0.0, 20.2, 64.8, 90.2]), ("tr", "translate", [24.8, 60.2])]
...
def test_the_callers_batch_size_reaches_the_decoder_call():
    """The route accepts batch_size and run_transcription used to drop it in kwargs, so
    every request ran at the engine's default whatever the caller asked."""
    mock_model = mock.MagicMock()
    regions, labels = _mixed_film(mock_model)
    with (
        mock.patch.object(model_manager.speech_clips, "vad") as vad_module,
        mock.patch.object(model_manager.segment_languages, "for_clips", return_value=labels),
        mock.patch.object(model_manager.gap_filling, "fill_language_gaps", side_effect=lambda _m, _p, segs, spans, **_k: (segs, spans)),
    ):
        vad_module.get_speech_timestamps_from_path.return_value = regions
        model_manager.run_transcription("film.wav", language=None, task="transcribe", vad_filter=True, batch_size=8)

    assert all(c.kwargs["batch_size"] == 8 for c in mock_model.transcribe.call_args_list)
