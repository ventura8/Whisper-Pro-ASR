...
class TestWhenDetectionIsPaidFor:
    ...
    def test_a_failure_on_a_later_chunk_keeps_the_labels_measured_before_it(self):
        """The first chunk's labels are as measured as they would have been alone; a
        detector that dies on the second chunk used to throw them away with it."""
        regions = [{"start": float(i), "end": float(i) + 0.5} for i in range(0, 20)]
        ...
        class _Detector(_RegionDetector):
            ...
            def detect_language_regions(self, audio_path, regions):
                """The second chunk is where the worker breaks."""
                if self.calls == 1:
                    self.calls += 1
                    raise RuntimeError("worker gone")
                return super().detect_language_regions(audio_path, regions)

        checks: list[int] = []
        spans = segment_languages.for_clips(_Detector(), "clip.wav", regions, preemption_check=lambda: checks.append(1))
        assert len(spans) == segment_languages.DETECTION_CHUNK, "the first chunk's labels survive"
        assert checks == [1], "the check after the first chunk ran; nothing after the failure"
    ...
    def test_the_workers_samples_are_released_after_the_last_chunk_only(self):
        """The worker keeps the file decoded across chunks -- and across a pause between
        them -- and is asked to drop it once, when the file's detection is over."""
        regions = [{"start": float(i), "end": float(i) + 0.5} for i in range(0, 40)]
        events = tuple({"start": r["start"], "end": r["end"], "result": {"detected_language": "es", "confidence": 0.9}} for r in regions)
        ...
        class _Detector(_RegionDetector):
            ...
            def release_region_audio(self, audio_path):
                """Record how many chunks had been streamed when the release came."""
                released_at.append((audio_path, self.calls))

        tracked = _Detector()
        checks: list[int] = []
        segment_languages.for_clips(tracked, "clip.wav", regions, preemption_check=lambda: checks.append(len(released_at)))
        assert tracked.calls == 3
        assert released_at == [("clip.wav", 3)], "released once, by path, after the third and last chunk"
        assert checks == [0, 0, 0], "never released before a pause between chunks"

    def test_the_samples_are_released_when_the_detection_fails_or_the_check_raises(self):
        """An early exit still releases them, and the release never masks the exception."""
        ...
        failing = _Detector()
        failing.fail_with = RuntimeError("detector broke")
        assert not segment_languages.for_clips(failing, "clip.wav", BOTH_REGIONS)
        ...
        with pytest.raises(RuntimeError, match="task cancelled"):
            segment_languages.for_clips(abandoned, "clip.wav", BOTH_REGIONS, preemption_check=abandon)
        assert abandoned.released == 1

    def test_a_failing_release_is_logged_not_raised(self, caplog):
        """A file left decoded in the worker is a cost, not a lost transcription."""

        class _Detector(_RegionDetector):
            def release_region_audio(self, audio_path):
                """The worker died between the last chunk and the release."""
                raise RuntimeError("worker gone")

        with caplog.at_level("WARNING"):
            spans = segment_languages.for_clips(_Detector(), "clip.wav", BOTH_REGIONS)
        assert len(spans) == 2
        assert "Could not release the worker's region audio" in caplog.text

    def test_an_in_process_engine_has_nothing_to_release(self, detector):
        """No worker, no samples held elsewhere: the hook is simply absent."""
        assert not hasattr(type(detector), "release_region_audio")
        assert len(segment_languages.for_clips(detector, "clip.wav", BOTH_REGIONS)) == 2
