...
class TestAPause:
    ...
    def test_a_whole_file_decode_without_the_vad_resumes_on_the_rest_of_the_file(self, monkeypatch):
        """``vad_filter=False`` asked for every sample decoded: the remainder is one clip from
        the pause point to the end of the file, and the VAD is never consulted."""
        model = _Model([_segment(0.0, 10.0, "a"), _segment(10.0, 12.0)], [_segment(10.0, 60.0, "b")])
        pending = iter([True])
        scan = mock.Mock(side_effect=AssertionError("the VAD must not run on a decode that turned it off"))
        monkeypatch.setattr(resumable_decode.vad, "get_speech_timestamps_from_path", scan)
        consume = _consume(lambda: next(pending, False), mock.Mock())
        results, _ = resumable_decode.decode(model, "f.wav", {"language": "en", "vad_filter": False}, None, consume=consume)
        assert model.calls[1]["clip_timestamps"] == [10.0, 60.0]
        assert [seg["text"] for seg in results] == ["a", "b"]

    def test_a_pause_with_nothing_left_returns_what_it_has(self):
        """The last clip was consumed and then a pause came: the audio past it is what the
        clips excluded, so no second call is made -- neither with an empty seek list (the
        decoder reads that as the whole file) nor for the tail, where it would invent speech."""
        model = _Model([_segment(0.0, 4.0, "a"), _segment(4.0, 5.0)], [])
        pending = iter([True])
        clips = [{"start": 0.0, "end": 4.0}]
        results, info = resumable_decode.decode(
            model, "f.wav", {"language": "en"}, clips, consume=_consume(lambda: next(pending, False), mock.Mock())
        )
        assert len(model.calls) == 1
        assert [seg["text"] for seg in results] == ["a"]
        assert info is model.info

    def test_a_whole_file_pause_with_no_speech_left_returns_too(self, monkeypatch):
        """No clips and the VAD finds nothing after the pause point: nothing to decode."""
        model = _Model([_segment(0.0, 10.0, "a"), _segment(10.0, 12.0)], [])
        pending = iter([True])
        monkeypatch.setattr(resumable_decode.vad, "get_speech_timestamps_from_path", mock.Mock(return_value=[]))
        consume = _consume(lambda: next(pending, False), mock.Mock())
        results, _ = resumable_decode.decode(model, "f.wav", {"language": "en", "vad_filter": True}, None, consume=consume)
        assert len(model.calls) == 1
        assert [seg["text"] for seg in results] == ["a"]
