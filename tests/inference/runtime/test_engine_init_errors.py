...
def _clear_errors():
    """Reset every piece of process-global state init_unit writes to.

    MODEL_POOL and PREPROCESSOR_POOL as well as LAST_INIT_ERROR: the successful-load test
    below leaves a MagicMock engine in the pool, and a later module reading it as a real
    engine fails somewhere unrelated to its own subject.
    """
    for pool in (model_manager.LAST_INIT_ERROR, model_manager.MODEL_POOL, model_manager.PREPROCESSOR_POOL):
        pool.clear()
    yield
    for pool in (model_manager.LAST_INIT_ERROR, model_manager.MODEL_POOL, model_manager.PREPROCESSOR_POOL):
        pool.clear()
