...
class TestAMonolingualFilm:
    ...
    def test_an_unlabelled_stretch_is_not_evidence_for_the_label_before_it(self):
        """One second of Ukrainian, then five seconds the detector could not name, in a
        Russian film. The unlabelled seconds continue the block but say nothing about its
        language; counted as switch evidence they made a one-line mislabel a six-second
        scene, and the film fractured on a label nobody measured."""
        regions = _regions((0, 10.0), (10.2, 11.2), (11.4, 16.4), (16.6, 26.0))
        labels = _labels((0, 10.0, "ru", 0.9), (10.2, 11.2, "uk", 0.9), (16.6, 26.0, "ru", 0.9))
        runs = language_runs.build(regions, labels)
        assert [(r["start"], r["end"], r["language"]) for r in runs] == [(0, 26.0, "ru")]
        assert len(runs[0]["regions"]) == 4, "the unlabelled stretch still rides in the run"

    def test_an_unlabelled_stretch_does_not_shorten_a_real_switch_either(self):
        """The same scene with three labelled seconds of Ukrainian is a switch, as before."""
        regions = _regions((0, 10.0), (10.2, 13.4), (13.6, 16.4), (16.6, 26.0))
        labels = _labels((0, 10.0, "ru", 0.9), (10.2, 13.4, "uk", 0.9), (16.6, 26.0, "ru", 0.9))
        runs = language_runs.build(regions, labels)
        assert [r["language"] for r in runs] == ["ru", "uk", "ru"]
        assert [r["start"] for r in runs[1]["regions"]] == [10.2, 13.6]
