...
def _join_and_assert_dead(thread: threading.Thread, timeout: float) -> None:
    thread.join(timeout=timeout)
    assert not thread.is_alive(), f"{thread.name} did not terminate before mocks/shared resources are restored"
...
def _run_cleaner_vs_init_race(
    ...
    try:
        ...
        release_unload.set()
        _join_and_assert_dead(t_clean, timeout=5.0)
        if t_init_started:
            _join_and_assert_dead(t_init, timeout=5.0)
