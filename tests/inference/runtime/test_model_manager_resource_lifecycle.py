...
class TestPreemptionAndPriority:
    ...
    def test_run_vocal_isolation_uses_assigned_accelerator_preprocessor_per_unit(self):
        ...
        # Colocation is decided from the detected hardware, not from whichever preprocessors
        # happen to have been built yet -- so a test whose premise is "two accelerator units
        # each run their own" has to say so where the routing actually looks. Populating only
        # the pool used to be enough, and that was the bug: at /status time a two-accelerator
        # host still looked single-accelerator and the first task genuinely routed off-unit.
        units = [{"id": "GPU", "type": "GPU", "name": "GPU unit"}, {"id": "NPU", "type": "NPU", "name": "NPU unit"}]
        with mock.patch("modules.core.config.HARDWARE_UNITS", units), mock.patch("modules.core.config.PREPROCESS_DEVICE", "NPU"):
