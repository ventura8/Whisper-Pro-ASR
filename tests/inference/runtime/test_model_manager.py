...
class TestLoadModel:
    ...
    def test_amd_units_are_isolated_through_the_generic_path(self):
        ...
        # engine_for_unit is patched here as it is in the neighbouring isolation tests.
        # Without it this test read the ambient config.ASR_ENGINE, so what it asserted
        # depended on how the resolved engine happened to fall out on the machine running
        # it -- and _should_isolate excludes WHISPERX, which would take the in-process path.
        with mock.patch("modules.core.config.ISOLATE_ENGINES", True):
            with mock.patch("modules.core.config.engine_for_unit", return_value="OPENAI-WHISPER"):
                with mock.patch("modules.inference.engines.isolated_engine.IsolatedEngine", return_value=proxy) as mock_isolated:
                    model_manager.init_unit(unit)
