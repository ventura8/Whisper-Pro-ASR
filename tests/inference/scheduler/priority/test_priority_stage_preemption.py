...
def _exercise_ffmpeg_stage_priority_scenario():
    """Run the FFmpeg drain scenario and return the observed state."""
...
def _exercise_inference_stage_scenario():
    """Run the inference yield scenario and return the observed state."""
    _setup_units(
        [
            {"id": "GPU.0", "type": "GPU", "name": "Intel GPU"},
        ...
        try:
            with model_manager.early_task_registration(is_priority=False):
                results = consume_transcription_segments(
                    ...
                    preemption_check=model_manager._check_preemption,
                )
...
def _exercise_triple_unit_matrix_priority_liveness(hw_list, std_task_count):
    """Run the triple-unit matrix scenario and return the observed state."""
    _setup_units(hw_list)
    events = []

    std_threads = [threading.Thread(target=_run_standard_hold, args=(events, f"s{i}", 3, 0.12)) for i in range(std_task_count)]
    ...
    return {
        "events": events,
        "standard_alive": any(thread.is_alive() for thread in std_threads),
        "priority_alive": t_prio.is_alive(),
        "std_task_count": std_task_count,
    }
...
def test_ffmpeg_stage_priority_does_not_wait_for_standard_ffmpeg():
    ...
    result = _exercise_ffmpeg_stage_priority_scenario()
    assert "prio_ffmpeg_wait" in result["events"]
    assert not result["priority_alive"]
    assert "prio_ffmpeg_waited" in result["events"]
    assert "prio_ffmpeg_done" in result["events"]
...
def test_vocal_separation_stage_invokes_yield_hook():
    ...
    result = _exercise_vocal_separation_stage_scenario()
    assert (
        result["standard_alive"],
        result["priority_alive"],
        "asr_vocal_done" in result["events"],
        "prio_vocal_done" in result["events"],
        result["hook_calls"] >= 3,
...
def test_inference_stage_invokes_yield_hook():
    ...
    result = _exercise_inference_stage_scenario()
    assert (
        result["standard_alive"],
        result["priority_alive"],
        "asr_infer_done" in result["events"],
        "prio_infer_done" in result["events"],
        result["hook_calls"],
...
def test_triple_unit_matrix_priority_liveness(hw_list, std_task_count):
    ...
    result = _exercise_triple_unit_matrix_priority_liveness(hw_list, std_task_count)
    assert not result["standard_alive"]
    assert not result["priority_alive"]
    assert "prio_matrix_done" in result["events"]
...
def test_asr_yields_to_detect_language_before_vocal_separation():
    ...
    with (
        mock.patch("modules.api.support.request_utils.initialize_task_context", return_value=("/tmp/in.wav", None, None)),
        mock.patch("modules.api.routes.asr._detect_lang_if_needed", return_value="en"),
        mock.patch("modules.api.routes.asr._check_preemption", side_effect=fake_preemption),
        mock.patch("modules.api.routes.asr.model_manager.run_transcription", side_effect=fake_run_transcription),
        mock.patch("modules.api.routes.asr.model_manager.update_task_progress"),
        mock.patch("modules.api.routes.asr.model_manager.update_task_metadata"),
        mock.patch("modules.api.routes.asr.config.ENABLE_VOCAL_SEPARATION", True),
        mock.patch("modules.api.support.request_utils.get_clean_wav_or_error", return_value=("/tmp/clean.wav", None)),
    ):
    ...
    assert (
        ...
        source_path,
        result["text"],
        "check_preemption" in events,
        "run_transcription" in events,
        events.index("check_preemption") < events.index("run_transcription"),
    ) == (None, "/tmp/in.wav", "ok", True, True, True)
...
def test_asr_yields_again_immediately_before_inference():
    ...
    with (
        ...
        mock.patch("modules.api.routes.asr.model_manager.update_task_progress"),
        mock.patch("modules.api.routes.asr.model_manager.update_task_metadata"),
        mock.patch("modules.api.routes.asr.config.ENABLE_VOCAL_SEPARATION", False),
        mock.patch("modules.api.support.request_utils.get_clean_wav_or_error", return_value=("/tmp/clean.wav", None)),
    ):
        result, source_path, err = routes_asr._perform_transcription_task(
            params,
    ...
    assert (
        ...
        source_path,
        result["text"],
        events.count("check_preemption") >= 2,
        events[-2:],
    ) == (None, "/tmp/in.wav", "ok", True, ["check_preemption", "run_transcription"])
