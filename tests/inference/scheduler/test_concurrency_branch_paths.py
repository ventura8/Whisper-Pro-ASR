...
def test_try_borrow_preemptible_unit_without_target_covers_known_and_unknown_ids():
    ...
    with (
        ...
        with mock.patch("modules.inference.runtime.concurrency._is_preprocessor_lock_available", return_value=True):
...
def test_is_preprocessor_lock_available_returns_false_when_lock_held():
    ...
    with mock.patch.dict(
        ...
        {"modules.inference.runtime.model_manager": fake_model_manager},
        clear=False,
    ):
...
def test_is_preprocessor_lock_available_allows_missing_model_manager_or_lock():
    ...
    with mock.patch.dict(
        concurrency.sys.modules,
        {"modules.inference.runtime.model_manager": None},
        clear=False,
    ):
        assert concurrency._is_preprocessor_lock_available("GPU.0") is True
...
def test_try_borrow_preemptible_unit_fallback_skips_locked_candidate_and_uses_next():
    ...
    with (
        ...
        mock.patch(
            "modules.inference.runtime.concurrency._is_preprocessor_lock_available",
...
def test_acquire_unit_for_task_waits_without_timeout_when_no_progress_possible():
    ...
    with (
        mock.patch("modules.inference.runtime.concurrency._try_acquire_unit_now", return_value=None),
        mock.patch("modules.inference.runtime.concurrency.time.sleep", side_effect=RuntimeError("stop-loop")),
    ):
...
def test_standard_task_acquires_idle_unit_while_priority_is_active_without_backlog():
    ...
    with (
        ...
        mock.patch("modules.inference.runtime.concurrency._try_acquire_unit_now", return_value={"id": "GPU.0"}),
        mock.patch("modules.inference.runtime.concurrency.time.sleep") as mock_sleep,
    ):
...
def test_standard_task_capacity_threshold_for_queued_priority(monkeypatch, accel_limit, queued_priority_count, expect_blocked):
    ...
    if expect_blocked:
        with (
            mock.patch("modules.inference.runtime.concurrency._try_acquire_unit_now", return_value={"id": "GPU.0"}),
            mock.patch("modules.inference.runtime.concurrency.time.sleep", side_effect=RuntimeError("blocked-loop")),
        ):
...
def test_determine_preemption_needed_includes_pause_generation():
    ...
    unit_id = next(iter(scheduler.STATE.unit_sync))
