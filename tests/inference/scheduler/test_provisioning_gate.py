...
def test_the_gate_runs_before_any_hardware_allocation(quiet_scheduler):
    ...
    # `fifo.assert_not_called(), "..."` builds a tuple and throws it away: the call ran, but
    # pairing it with a string made the statement an expression whose value nothing checks.
    assert not fifo.called, "the FIFO check is downstream of the gate"
