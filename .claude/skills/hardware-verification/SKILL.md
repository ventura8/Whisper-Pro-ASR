...
The override must match the target under test. Hardcoding the NVIDIA one is how a run
"validates" an Intel or AMD target on a CUDA-less overlay and then reports a clean CPU
fallback as a pass:

```bash
BUILD_TARGET=nvidia   # the target you are actually validating
...
Two targets need a second overlay (`intel-xpu`, `amd-rocm-torch`) -- see `docs/SETUP.md`.
...
The fastest signal, ~1 minute. This one file, not a pipeline stage: no `PIPELINE_STAGE`
isolates it, and pointing this step at `real-audio` would run exactly the same suite as
step 4 and prove nothing step 4 does not.

It is an HTTP client against the **running** container, so it needs `--network host` and a
base URL rather than the app stack -- run it from the test image so the client libraries are
the pinned ones:

Build the commit-tagged image first, as the Docker section below does -- `:latest` is
reassigned by every build, so running it here can validate a tree that is not the one
checked out. `--pull=never` keeps a missing local image an error rather than a silent pull:

```bash
TEST_IMAGE="whisper-pro-asr-test:$(git rev-parse --short HEAD)"
docker build -f Dockerfile.test --target test -t "$TEST_IMAGE" .

docker run --rm --pull=never --network host -e RUN_REAL_ASR=1 -e HOME=/tmp \
  -e WHISPER_BASE_URL=http://127.0.0.1:9000 -u "$(id -u):$(id -g)" \
  -v "$PWD:/app" -w /app "$TEST_IMAGE" \
  python3 -m pytest tests/integration/test_transcription_accuracy.py -ra
```
...
smoke set would have caught inside its 20-minute budget.
...
Use `intel_gpu_top` for an Intel **GPU**. It does not cover the **NPU** — the NPU is a
separate device with no engine rows in `intel_gpu_top`, so a busy NPU reads there exactly
like an idle one, and reading it as evidence has three times signed off a silent CPU
fallback. For the NPU, ask the running service what its engine settled on
(`/status` reports the per-unit execution device, and a unit that fell back is labelled as
such), and confirm the device is present and claimed:

```bash
docker exec whisper-pro-asr python3 -c "import openvino; print(openvino.Core().available_devices)"
ls -l /dev/accel/
```

On AMD, use `rocm-smi` — the container
...
docker run --rm --pull=never --network host -v "$PWD:/app" -w /app -u 1000:1000 \
...
- No `--gpus all` here, deliberately. These tests drive a service that is already running
  with its own device access; the test container only speaks HTTP. Passing it on a CPU,
  Intel or AMD host fails outright with "could not select device driver", so it belongs
  only on the long-form command below, whose gate checks for `nvidia-smi` inside the
  container:
  ...
  docker run --rm --pull=never --gpus all --network host -v "$PWD:/app" -w /app -u 1000:1000 \
    ...
    "$TEST_IMAGE" /bin/bash -c 'python3 -m pytest tests/real_audio/test_longform_stress.py -ra --no-cov'
  ```
...
```bash
# --network host and RUN_REAL_ASR=1 are both load-bearing: this stage drives the *running*
# service, and without the gate variable every real-audio test skips, which the stage then
# fails as exit 5 (no collection) rather than reporting a pass.
docker run --rm --pull=never --network host -e WHISPER_PRO_ASR_TEST_IMAGE=1 \
  -e RUN_REAL_ASR=1 -e WHISPER_BASE_URL=http://127.0.0.1:9000 -e HOME=/tmp \
  -e PIPELINE_STAGE=real-audio \
