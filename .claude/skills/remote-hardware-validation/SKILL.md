...
```bash
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "docker run --rm --gpus all $IMAGE python3 -c \"import ctranslate2,logging;logging.basicConfig(level=logging.INFO,format='%(message)s');logging.getLogger('ct2_probe').info('ct2 cuda devices: %s', ctranslate2.get_cuda_device_count())\""
```
...
```bash
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "docker run --rm --device /dev/kfd --device /dev/dri $IMAGE rocm-smi --showhw"
```
...
A fixed `/tmp/xpu_probe.py` on both ends is a collision waiting to happen -- two probes at
once, or a stale file from an interrupted run mounted into a later container. Give each run
its own name on each host, and remove both however the run ends:

```bash
LOCAL_PROBE="$(mktemp -t xpu_probe.XXXXXX)"
...
The cleanup ssh carries the same BatchMode/ConnectTimeout options as every other
connection here, and for the same two reasons: a trap that prompts for a password
blocks a non-interactive run at the moment it is trying to exit, and one with no
connect timeout hangs indefinitely against a host that has since gone away --
leaving the probe behind on the remote anyway, which is what the trap exists to prevent.
...
EOF
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "cat > $REMOTE_PROBE" < "$LOCAL_PROBE"
# The render node comes from the audit. renderD128 is only the most common first one;
# numbering starts higher when other DRM devices are present, so glob it and reuse the one
# path for both the --device mapping and the GID lookup.
RENDER_NODE="$(ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> 'ls -1 /dev/dri/renderD* | head -n 1')"
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "docker run --rm --device ${RENDER_NODE}:${RENDER_NODE} \
...
- **Disk headroom** — targets run 4.9 GB (`cpu`) to ~29.8 GB (`full`), plus
