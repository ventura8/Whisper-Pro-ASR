...
```bash
# Unique paths on both ends, removed by a trap. A fixed /tmp/xpu_probe.py is world-writable
# and shared: two validations against one host, or two operators, overwrite each other's
# probe mid-run, and a stale one left behind is picked up by the next.
LOCAL_PROBE="$(mktemp)"
REMOTE_PROBE="/tmp/xpu_probe_$$_${RANDOM}.py"
trap 'rm -f "$LOCAL_PROBE"; ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "rm -f $REMOTE_PROBE" >/dev/null 2>&1' EXIT INT TERM
...
Success is the `XPU INFERENCE OK` line the probe logs; anything else, including a clean
exit with no such line, is a failure.
