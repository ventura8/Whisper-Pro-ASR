...
30, tier-A 28 + 2 xfailed, long-form 44. The RTX 5090 (`ASR=CUDA measured=True`,
`Resource Pool: cuda:0`, float16), same tree: the full real-audio suite **396 passed, 11
xfailed, 3 xpassed** (the same three flappers as W8 -- `ka_tail`, `te_tail`, `it_core`'s
translation -- and `uk_core`'s translation xfailed as recorded), the six long-form
variants plus both translations **44 passed**, and the pause/resume drive below.
...
Run again with Bazarr paused (N8c, release tree, 2026-09-13), and this time as the
long-form stress *test* rather than a bare request, so its assertions ran: three
detections fired by hand at +601 s, +1802 s and +2402 s returned in 60 s, 30 s and 30 s
(five more still arrived from outside and returned too); the decode paused twice --
`Paused after 13 segment(s)` / `Resuming: 12 clip(s) left to decode`, `Paused after 14` /
`Resuming: 11` -- and the transcription finished in 45:35, its translation in another
45:32. **7 of 8 assertions pass**: `windows`, `quiet`, `fracture`, `language`, coverage,
repetition and translation all hold across the pauses; the one failure is `throughput`
at RTF 2.27, which a CPU host always fails. Plan row N8 is done.

The same drive on the RTX 5090 (`cuda:0`, vocal separation on, 2026-09-13): detections
fired at +20 s, +60 s and +100 s returned in 16 s, 13 s and 7 s, and the transcription
finished in 277 s with 225 segments in all ten languages. All three landed in the
transcription's own UVR pass -- the second deadlock's path -- and the log shows it doing
what the NUC never got to: `Paused during Vocal Separation; the preprocessing worker is
released for the priority task`, then `Resuming Vocal Separation from the start`, three
times, before `Starting transcription on hardware unit cuda:0` and the ten per-language
calls. The decode-phase pause is the NUC's evidence above; the separation-phase pause is
this one.
