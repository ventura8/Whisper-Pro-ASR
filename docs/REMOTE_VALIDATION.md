...
**What the service does now.** `device_probe.npu_can_execute()` runs before the banner
prints. It does not execute the model and does not spawn a subprocess -- it reads the
OpenVINO IR's *metadata* in the main process, because `read_model` parses XML and creates no
device context, and because actually compiling Whisper on the NPU overran a 180-second
budget on the one attempt that tried it. Dynamic input shapes in the IR are the whole
answer, and they are visible in milliseconds.

If the NPU cannot execute, `config.py` sets `DEVICE` and `ASR_DEVICE` to **CPU** -- not the
iGPU -- and drops the NPU from the ASR scheduler pool, before anything is reported:

```text
[System] ASR_DEVICE=NPU cannot execute this model: ...
[System] ASR falls back to the CPU; the NPU stays available for UVR preprocessing.
```

CPU rather than the GPU is deliberate: on a machine with both, sending this unit to the iGPU
would put ASR for both units on one device and serialise them. The NPU keeps doing the one
thing it is good at -- UVR preprocessing -- while ASR for it runs on the CPU, so the two
proceed in parallel.
...
- **Windows** — SSH lands in PowerShell or `cmd`, where `uname`, `df` and the container
  ...
  ```bash
  ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> 'wsl -e bash -lc "<command>"'
  ```

  The same options as the audit command above, and for the same reasons: the dedicated key
  because that is the only identity `remote_validate.sh` offers, and `BatchMode=yes` because
  a password prompt in an automated session hangs rather than failing.
...
```bash
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> 'bash -s -- --json' < scripts/audit_hardware.sh
```
...
Both copies of the probe get unique paths and are removed on exit. A fixed
`/tmp/xpu_probe.py` is world-writable-directory state under a predictable name: two
validations of the same host at once overwrite each other's probe, and every run that ends
early leaves the file behind for the next one to silently reuse.

```bash
LOCAL_PROBE="$(mktemp -t xpu_probe.XXXXXX.py)"
REMOTE_PROBE="/tmp/xpu_probe.$$.$RANDOM.py"
trap 'rm -f "$LOCAL_PROBE"; ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> "rm -f $REMOTE_PROBE" >/dev/null 2>&1' EXIT
cat > "$LOCAL_PROBE" <<'EOF'
...
EOF
ssh <user>@<host> "cat > $REMOTE_PROBE" < "$LOCAL_PROBE"
IMAGE='whisper-pro-asr@sha256:<digest of the build under test>'
# The render node is whatever the hardware audit reported. renderD128 is only the most
# common first one; numbering starts higher when other DRM devices are present, so glob it
# and reuse the one path for both the --device mapping and the GID lookup.
RENDER_NODE="$(ssh <user>@<host> 'ls -1 /dev/dri/renderD* | head -n 1')"
ssh <user>@<host> "docker run --rm --device ${RENDER_NODE}:${RENDER_NODE} \
  --group-add \"\$(stat -c %g ${RENDER_NODE})\" -u root \
  -v $REMOTE_PROBE:/probe.py $IMAGE python3 /probe.py"
```
...
| 5–30 GB free, plus ~3 GB per model format | Targets range from `cpu` (~4.9 GB) to `full` (~29.8 GB) |

Verify the whole chain in one command:

```bash
ssh -i ~/.ssh/whisper_remote_validation -o IdentitiesOnly=yes -o BatchMode=yes -o ConnectTimeout=10 <user>@<host> 'id -nG | tr " " "\n" | grep -qx docker && echo ACCESS OK'
```
...
## Hybrid NVIDIA + Intel: the Intel iGPU is never used (2026-09-04, fixed in v1.3.0)

> Both defects in this section -- the unit never entering the pool, and the ONNX variant
> then denying it acceleration -- are fixed. The investigation is kept because the
> measurements and the reasoning are what the fixes rest on; read to the corrections at the
> end before acting on anything here.
...
thing); only UVR and VAD do.

### Correction: fixed in v1.3.0 (2026-09-07)

The paragraph above ended by saying the variant ordering "was not attempted here". It has
been, and the guidance that followed it -- leave `ASR_PREPROCESS_DEVICE` unset on a hybrid
host -- is **no longer necessary**.

`bootstrap._intel_preprocess_is_reachable` now asks whether an explicitly requested Intel
preprocess device can actually be *served* before that request selects the ONNX Runtime
variant. It cannot when ASR runs on CUDA in the same interpreter: `provider.py` blocks that
combination, and the Intel build ships no CUDA provider for the block to substitute, so the
request degraded to `CPUExecutionProvider` while every log line still named the iGPU. When
the request is unreachable the resolution falls through to `auto`, the CUDA variant loads,
and the block substitutes a provider that exists.

Measured on the RTX 3080 + UHD Graphics laptop with `ASR_PREPROCESS_DEVICE=GPU` set
explicitly -- the configuration that previously produced the CPU fallback:

| property | before | after |
| --- | --- | --- |
| UVR ONNX provider | `CPUExecutionProvider` | **`CUDAExecutionProvider`** |
| isolation speed | 0.33x | **2.61x** |
| request wall clock | 30-60s | **12.6s** |

Consistent with the 0.48x/2.40x pair recorded above; the spread is run-to-run variance on
the same machine, not a different effect.

Two things this does **not** change. The ordering still keys off `ASR_DEVICE` rather than
`ASR_PREPROCESS_DEVICE` -- the fix makes an unreachable request fall back cleanly instead of
reversing the precedence. And it applies only to preprocessing that shares the interpreter
with CUDA: an *isolated* UVR worker is started with `CUDA_VISIBLE_DEVICES=""`, so no CUDA
context can exist in it, and `provider.py` leaves OpenVINO in place there. That isolated
CUDA+Arc arrangement is reasoned from the isolation environment rather than measured -- no
host with both an NVIDIA GPU and an Arc-class Intel GPU was available.

### Correction to the correction: the guard did not engage by default (2026-09-08)

The measurement above is real but was taken with `ASR_ISOLATE_PREPROCESSING=0`. The guard
as first written asked whether preprocessing had been *requested* in isolation and returned
early if so -- and isolation is on by default, so on a default container the guard never
ran at all. The 2.61x figure therefore described a configuration almost nobody uses.

Two facts had to be separated. Isolation is requested up front; whether an isolated worker
is what actually serves the request is decided later, and the Intel variant was already
selected by then. The clause was removed: reachability now depends only on whether the ASR
side would take CUDA in this interpreter.

Re-measured on the same laptop with **no isolation override** -- the shipped default:

| property | before | after |
| --- | --- | --- |
| ONNX variant chosen | `/app/libs/intel` | **`/app/libs/nvidia`** |
| UVR speed | 0.90x | **8.34x** |
| `/status` UVR device | CPU (measured) | **CUDA (measured)** |

The 0.90x is the number a default v1.2.2-style container was really getting; 2.61x was
