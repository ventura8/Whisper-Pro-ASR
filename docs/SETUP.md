...
# (or set it by hand -- run this INSTEAD of the audit, not after it. .env is read
#  top-to-bottom with the last assignment winning, so appending a second BUILD_TARGET
#  line silently overrides whatever the audit just detected:)
...
# 3. Resolve BUILD_TARGET to its overlay files. Most targets are one file, but two are not:
#    intel-xpu declares no devices of its own and REQUIRES docker-compose.intel.yml as its
#    parent (on its own it starts with no render-group membership, so the XPU torch build
#    finds no device and openai-whisper silently runs on the CPU), and any Intel target
#    additionally needs docker-compose.intel-npu.yml when NPU preprocessing is wanted --
#    `devices:` is mandatory in Compose, so /dev/accel cannot be listed unconditionally.
#    There is no default here: docker-compose.nvidia.yml on an Intel, AMD or CPU host
#    either fails on a device that does not exist or starts an image with no passthrough.
...
case "$BUILD_TARGET" in
  intel-xpu) OVERLAYS="-f docker-compose.intel.yml -f docker-compose.intel-xpu.yml" ;;
  *)         OVERLAYS="-f docker-compose.${BUILD_TARGET}.yml" ;;
esac
# Add the NPU passthrough only on an Intel-bearing target whose host actually has one.
case "$BUILD_TARGET" in
  intel*|nvidia-intel|full)
    [ -e /dev/accel ] && OVERLAYS="$OVERLAYS -f docker-compose.intel-npu.yml" ;;
esac
docker compose -f docker-compose.yml $OVERLAYS up -d --build
```
...
Radeon parts available for testing were two `gfx1036` integrated ones -- a discrete-host APU
and the RTX 5090 box's Granite Ridge graphics -- so that is the one row backed by
measurement, and it is the negative result. Both produced the same false positive
(`/status` reporting `UVR = AMD GPU, measured: true, fallback: false` while the work ran on
the CPU), which makes it a property of running an unlisted architecture rather than one
machine's quirk. Do not read ❔ as working.
...
#    cpu | intel | intel-xpu | nvidia | nvidia-intel | nvidia-whisperx | amd | amd-rocm-torch | full
# Same overlay resolution as the Method 2 build above: intel-xpu requires
# docker-compose.intel.yml as its parent, and Intel targets add docker-compose.intel-npu.yml
# when the host has an NPU. BUILD_TARGET is read without executing .env, exactly as there.
BUILD_TARGET="$(sed -n 's/^[[:space:]]*BUILD_TARGET[[:space:]]*=[[:space:]]*//p' .env | tail -n1 | tr -d '"'"'"'\r')"
case "$BUILD_TARGET" in
  cpu|intel|intel-xpu|nvidia|nvidia-intel|nvidia-whisperx|amd|amd-rocm-torch|full) ;;
  *) echo "BUILD_TARGET in .env is missing or unsupported: '$BUILD_TARGET'" >&2; exit 1 ;;
...
docker compose -f docker-compose.yml $OVERLAYS up -d
