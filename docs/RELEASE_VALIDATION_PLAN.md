...
| 1.2 | **Nine cold builds** from an empty cache | all nine succeed | **done** -- all nine succeed from an empty cache on `local`: cpu 28 min, intel 30, intel-xpu 132, nvidia 64, full 29, nvidia-intel 46, amd 42, amd-rocm-torch 30; `nvidia-whisperx` failed once at 20 min on a transient fetch and succeeded on retry (39 min) |
...
| R3 | nvidia | FASTER-WHISPER | CUDA | CS / OPEN / END / EP / DUB excerpts | Code-switch localisation against forced tracks, invented text in title music and credits, whole episodes, dub tracks | **done as R4** -- the 54-excerpt probe below covers CS / OPEN / END / EP / DUB |
...
| L11 | nvidia | FASTER-WHISPER | CUDA | translation: tier-A 10 clips, code-switched 6, long-form stress + film | Every language in a file is translated from itself -- the per-language decode carries `task=translate`, and follows switches even when the caller named a language | **done** -- first run (RTX 3080, 2026-09-12) 16 of 18: both long-form translations reach every window in English, all six code-switched clips come back with both legs translated and no foreign words left; `it_core` (0.44) and `uk_core` (0.31) fall under the 0.50 bar on the model's own translation of a six-second clip and are recorded as translation-scoped defects. Re-run on the RTX 3080 at every later commit (28 passed, 2 xfailed) and in the RTX 5090's full suite (W8, W9), where `it_core` cleared the bar and `uk_core` did not; `it_core` is recorded as a flapper like `ka_tail` |
...
| N8 | cpu | FASTER-WHISPER | CPU | stress clip with a `/detect-language` fired mid-transcription, `--timeout 10800` | The deadlock the NUC found on 2026-09-12 (a paused task held the worker channel the priority detection needed) is closed: the detection completes while the transcription is paused, the transcription resumes and finishes with every assertion holding | **done as N8c** (release tree, 2026-09-13, Bazarr paused): the long-form stress *test* itself run on the NUC with three priority detections fired into it by hand at +601 s / +1802 s / +2402 s -- returned in 60 s / 30 s / 30 s -- plus five that still arrived from outside; `Paused after 13 segment(s)` / `Resuming: 12 clip(s)`, then `Paused after 14` / `Resuming: 11`, inside the decode; the transcription finished (45:35) and so did its translation, **7 of 8 assertions pass** -- `windows`, `quiet`, `fracture`, `language`, coverage, repetition and translation -- and the one failure is `throughput` at RTF 2.27, as a CPU host always fails it. The earlier N8b drive (same evidence for the pauses, transcription starved by an unpaused Bazarr) is superseded |
...
| W3 | nvidia | FASTER-WHISPER | CUDA | **full** (156 languages) | No regression anywhere in the matrix; only affordable here | **done as W3b / W8 / W9** -- the full matrix on the shipped design and twice on the pushed tree (below) |
...
| W5 | nvidia | OPENAI-WHISPER | CUDA | smoke | Engine non-regression through the changed route | **done** -- 1 failed, 23 passed: `mix_en_fr` content at 0.50, French leg only. The raised bar caught a third engine dropping a leg; W5b measured all six before the manifest was scoped |
...
| W6 | full | FASTER-WHISPER | AUTO | smoke | The image `latest` is meant to resolve to | **done as W6b / W6-local** -- `docker-compose.full.yml` lists `/dev/kfd`, which WSL2 has not, so the full image cannot start under its own override there (the file says so); the `--compose nvidia` runner flag exists for it. First attempt hung in pip for 4h; the retry built in 30 min and failed at `up` |
...
| W6b | full | FASTER-WHISPER | CUDA | smoke, `--compose nvidia` | The full image -- what `latest` resolves to -- started on a CUDA-only host | **done as W6-local** -- BuildKit on the WSL2 host died with SIGBUS mid-build (the host's Docker, not the tree; third failure of this image on this box that day). Moved to the local hybrid host, `--target full --compose nvidia-intel`: **25 passed**, image rebuilt from the shipped tree, `ASR=CUDA measured=True` -- the image `latest` resolves to starts and passes on a host without `/dev/kfd` when given the right override |
...
| W9 | nvidia | FASTER-WHISPER | CUDA | full real-audio suite at the final tree (`7d072be`), `--timeout 3600` | The whole matrix on the release commit, `resumable_decode` included | **done** (2026-09-13, `ASR=CUDA measured=True`, `cuda:0`): 396 passed, 44 skipped (long-form, gated on `RUN_GPU_LONG_ASR`), 11 xfailed, 3 xpassed -- the same three flappers as W8; `uk_core`'s translation xfailed as recorded |
| W10 | nvidia | FASTER-WHISPER | CUDA | longform, six variants + two translations, final tree | Every long-form assertion on Blackwell at the release commit | **done** (2026-09-13): 44 passed in 20:37, `cuda:0` |
| W11 | nvidia | FASTER-WHISPER | CUDA | stress clip with `/detect-language` fired at +20 s, +60 s, +100 s, separation on | The pause-and-resume on a GPU: every detection returns, the transcription finishes | **done** (2026-09-13): detections returned in 16 s / 13 s / 7 s; all three landed in the transcription's UVR pass -- `Paused during Vocal Separation; the preprocessing worker is released` / `Resuming Vocal Separation from the start` three times -- then the transcription finished in 277 s with 225 segments in all ten languages |
...
- [x] Phase 1.1 green
- [x] Phase 1.2: nine cold builds
- [x] Every Phase 2 row done, each with both claims evidenced (the AMD rows remain what the limitations below say: `gfx1036`-class silicon only)
- [x] Release notes reconciled against the actual numbers
- [x] Known Limitations still true -- the four in the release description (WhisperX and OpenAI-Whisper one language per file, the cost table, a single foreign line, a window is one line) each state a measured number that still holds at the final tree
- [x] `flavor: latest=false` in CI **and** `latest` claimed by `full` alone, on main and on a release tag -- on v1.3.0 the metadata action's `latest=auto` let all nine targets race for it and `amd-rocm-torch` won
- [ ] Commit, then tag
