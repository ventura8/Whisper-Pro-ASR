...
- **Graceful Token Fallbacks**: Diarization is fully optional and requires a Hugging Face Hub token (`DIARIZATION_HF_TOKEN`). If the token is missing or diarization fails, the system automatically falls back to standard transcription.
...
- **Inactivity Reclamation**: Introduces `MODEL_IDLE_TIMEOUT` configuration (in seconds). A deferred `threading.Timer` is scheduled once the last active session ends, unloading the warm models from RAM/VRAM only if the idle period elapses with no new work; any new task arriving before it fires cancels and reschedules the timer, avoiding aggressive/instant offloading between consecutive API requests.
