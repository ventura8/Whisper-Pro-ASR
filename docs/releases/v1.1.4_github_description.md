...
- **No More `xx (Unknown)`**: Previously, unrecognized or plain subtitle filenames would fall back to `xx (Unknown)` in Plex. The new naming convention avoids that hardcoded placeholder whenever a language value is available.
- **Language Auto-Detection**: The language code is resolved from the Whisper inference result, falling back to the user-specified `language` parameter when the inference result doesn't provide one; if neither yields a valid value, filename generation falls back to `en` rather than a hardcoded "unknown" placeholder.
...
HTTP API endpoints and existing configuration variables remain compatible; `SUBTITLE_PROMO_ENABLED`, `SUBTITLE_PROMO_TEXT`, and `SUBTITLE_PROMO_DURATION` are newly added in this release. Monitoring charts now report Intel GPU saturation at `100%` when the device is fully busy, and Python import usage changed due to the import-path migration even though the API surface stayed stable.
