...
- **Global VAD Scan**: Replaced redundant segment-level VAD passes with a single Global VAD pass, cutting VAD overhead by roughly an order of magnitude (≈9× fewer VAD passes).
- **Audio Standardization**: All incoming media is standardized to **16kHz, mono, 16-bit PCM WAV** via FFmpeg 8.1.0 for consistent accuracy across all inference engines.
...
- **Nuclear RAM Reclamation**: Integrated `malloc_trim(0)` and `ctranslate2.clear_caches()` into the offload cycle, prompting the OS to reclaim unused model memory as part of task completion/offload.
...
---
*For deployment instructions, refer to the [README.md](../../README.md) or [ARCHITECTURE.md](../ARCHITECTURE.md).*
