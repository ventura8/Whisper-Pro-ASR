...
- **AMD GPU (ROCm)**: Leverages `/dev/kfd` and `/dev/dri` on Linux, and `/dev/dxg` (the WSL2 GPU bridge) when running under WSL2 -- the same ROCm stack either way, not a separate DirectML path. `onnxruntime-rocm` is isolated under `/app/libs/amd` and loaded automatically when AMD hardware is detected; published images ship ROCm kernels for consumer Radeon RDNA2/RDNA3/RDNA4. What runs where differs by target: on `amd`, UVR vocal isolation offloads to the AMD GPU through ONNX Runtime ROCm while Whisper ASR stays on the CPU, because CTranslate2 has no ROCm backend. The opt-in `amd-rocm-torch` target adds a ROCm PyTorch build, which puts OpenAI-Whisper on the GPU as well; Faster-Whisper remains on the CPU there too.
