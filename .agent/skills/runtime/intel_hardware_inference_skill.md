...
- If an OpenVINO model directory fails structural integrity during pipeline initialization, runtime purges it and makes one bounded pipeline reload attempt. A failed purge or reload returns to the original initialization failure without retrying indefinitely.
...
- Hybrid CUDA+Intel images keep the system OpenCL ICD loader before CUDA's bundled `libOpenCL.so`; reversing that order masks `intel.icd` and makes OpenVINO report only `CPU` despite a usable mapped GPU.
