...
- **AMD Concurrency Limit**: `modules.core.config.get_parallel_limit('AMD')` honors the number of registered AMD units (capped by `MAX_AMD_UNITS`).
- **Auto-Detection Triggers**: `_detect_amd_hardware` checks `/dev/kfd` presence or AMD DRM vendor `0x1002` in `/sys/class/drm`. On WSL2 (`/dev/dxg`), mounted AMD WSL driver artifacts (`amdxc64.so` in the INF folder or one nested Adrenalin subdirectory) and `librocdxg.so` are strictly required to prevent false positives when running NVIDIA or Intel containers.
...
1. If both NVIDIA and AMD detected → load `/app/libs/amd` for AUTO and dual-GPU hosts (ONNX gets ROCm; CTranslate2 gets CUDA directly). Explicit NVIDIA-only preprocess is not required; presence of both GPUs selects the AMD ONNX library.
...
- **Utilization Charts**: Activity-inferred via `_resolve_amd_utilization` in `metrics_discovery.py`
  - Reports `100%` when AMD UVR stages are active or an accelerated preprocessor lock is held (ROCm / MIGraphX / DirectML providers recognized)
  - Does **not** treat AMD ASR/transcription as accelerator work (CTranslate2 runs on CPU for AMD units)
  - Reports `0%` when idle
  - Sysfs probes use AMD-vendor (`0x1002`) DRM cards mapped by unit index; mixed Intel/AMD hosts never fall back to `card*` wildcards
...
# Or run the Docker test image directly when you need the targeted suite:
...
docker run --rm -e CI=true -v "$PWD/assets:/app/assets" -v "$PWD/reports:/reports" \
  whisper-pro-asr-test /bin/bash -lc "python3 -m pytest tests/unit/test_config_amd.py"
```
