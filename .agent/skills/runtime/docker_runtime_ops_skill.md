...
6. Confirm build-time integrity checks:
   ...
   - **Never call bare `apt-get purge`/`install` from `strip_build_artifacts.sh`.** That
     script ends with `rm -rf /var/lib/apt/lists/*` and runs again in every target's final
     layer, so by then the package index is empty and apt cannot resolve a name at all:
     `E: Unable to locate package gcc`, exit 100, and under `set -euo pipefail` the build
     dies. That is a different outcome from "known but not installed" (`is not installed,
     so not removed`), which apt does tolerate -- conflating the two broke every standalone
     target in v1.3.0. Select the installed subset with `dpkg-query` first and purge only
     that, which keeps a genuine purge failure fatal.
   - This class of break hides behind the build cache: a target's final-stage `RUN` only
     re-executes when the build context changes, and CI's `cache-from: type=gha` masks it
     the same way. A cold build is the only thing that proves it. Verify all nine targets
     without materializing images:

     ```bash
     docker buildx build --target <t> --build-arg RUNTIME_BASE=<base> --output type=cacheonly .
     ```

     `RUNTIME_BASE` is the OpenVINO runtime for `intel`, `intel-xpu`, `nvidia-intel` and
     `full`, and pinned Ubuntu for the rest -- the same split `.github/workflows/ci.yml`
     makes in its `build-targets` matrix.
     ...
     - `cpu` (~4.9 GB): CPU only, no accelerator stack.
     - `intel` (~5.2 GB): Intel iGPU/Arc/NPU via OpenVINO.
     - `nvidia` (~17.5 GB): NVIDIA CUDA.
     - `nvidia-intel` (~17.9 GB): hybrid NVIDIA + Intel.
     - `amd` (~14.1 GB): AMD ROCm.
