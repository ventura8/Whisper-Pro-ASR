...
4. Test coverage: `>= 90%` overall and per file (current baseline higher, verified inside the Docker test image). The measured tree is `modules/`, `whisper_pro_asr.py` **and `scripts/`** -- the last was omitted by `.coveragerc` until v1.4.1, which is why its files could sit at 37%. Do not reintroduce that omit: the fixture generator, the audio catalog and the model preloader are ordinary source, and three security fixes landed there untested because nothing measured them.
...
11. SonarQube Cloud quality gate pass on every pull request. `sonar-project.properties` holds the analysis configuration and is shared by CI and `scripts/sonar_scan_local.sh`, so a local scan applies the same rules to the same files. Three points are load-bearing and were each found the hard way:
    - The `sonarqube` job runs only for pull requests and for `main`, release branches and tags. Analysing a non-main branch is not available on this plan, so the push-triggered run had no gate to report and failed on every push while the PR run beside it passed.
    - coverage.py records the roots it measured from, and the suite runs in the container at `/app`. The report is rewritten to an **absolute** workspace root before the scan; a relative `.` resolved `modules/` (whose entries hang off the `/app/modules` root) and silently failed for `scripts/`, so half the report imported and SonarCloud showed 0% on files measured at 100%.
    - The quality-gate step is skipped when either coverage-producing stage did not succeed. The scan itself stays `always()` -- findings matter most when something failed -- but a stage that never ran uploads no report, and the gate reads that absence as 0.0% coverage on new code, reporting a coverage regression whose real cause is upstream.
12. Concurrency-affecting changes must include liveness tests (pause/resume, queued waiting behavior, acquisition behavior) and pass related scheduler suites.
13. Concurrency-affecting changes must include synchronized documentation updates (`README.md`, `docs/CONCURRENCY.md`, and relevant `.agent/skills` files).
14. **Docker-Only execution policy**: Local parity scripts (`scripts/ci/build-and-test.sh`, `scripts/ci/build-and-test.ps1`) must not run eslint, stylelint, hadolint, shellcheck, taplo, bandit, or pip-audit on the host. They must build the Docker test image and run all tests/check output exclusively via that image.
15. Frontend verification and debugging must use Playwright CLI and MCP tooling when available; do not rely on manual-only browser validation.
16. Node.js dependencies and Playwright Chromium must be bootstrapped inside the Docker image for testing.
17. Python source code cyclomatic complexity must have 100% A-grade ranks (Radon cc rank A, score <= 5) for all functions, methods, and blocks.
18. The Docker test image pipeline must fail when any rank-B-or-worse function/block is detected. `run_radon_complexity_gate` uses a **single** `radon cc -s` invocation: output is piped to `tee complexity_output.txt` (the report artifact), then `grep -E ' [B-F] '` on that file detects violations without re-running radon over the source list.
19. `tests/run_suite.sh` inside the Docker test image must execute Radon complexity summary and rank-A enforcement in the `lint` stage, before any test stage (`js-unit-tests`, `python-tests`, `e2e-fixture`, `e2e-real`).
20. In Docker test images, Radon source enumeration must be filesystem-based (e.g., `find ... -name '*.py'`) and must not depend on `.git` metadata.
21. Dockerfile lint gate must pass with Hadolint (`hadolint --failure-threshold warning --disable-ignore-pragma Dockerfile Dockerfile.test`) inside the Docker test image.
22. PowerShell script lint gate must pass with PSScriptAnalyzer inside the Docker test image.
23. Shell script lint gate must pass with ShellCheck **and shfmt** inside the Docker test
