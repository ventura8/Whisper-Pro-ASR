...
`python3 scripts/generate_audio_matrix.py verify` reports which manifest entries the fixture
set covers. It is a fixture-authoring aid, not a gate: it runs on the host, asserts nothing
about the service, and passing it says nothing about whether a transcription is correct.
Listing it beside the two commands above reads as a third gate step.
