...
## Mandatory Thread-Closure Response Rule

- Before closing any unresolved PR thread, always post a detailed response comment in the thread.
- The response must include: issue summary, what changed (or why no code change was needed), validation performed, and any follow-up notes.
- This rule applies to all feedback levels, including nitpicks and minor comments.
- Never resolve a thread silently.
- Always keep monitoring for newly posted comments and respond to them as they arrive; continue this loop until the PR has zero unresolved threads.
...
LOOP (max 10 iterations):
  ...
  2. Detect newly posted comments/threads since the previous iteration
  3. Categorize by type (Docstring, Code Quality, Testing, etc.)
  4. Apply targeted fixes to each category
  5. Run full test suite & coverage validation
  6. Post progress comment to PR
  7. Count unresolved items
