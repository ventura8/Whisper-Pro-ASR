...
# Processes actionable bot threads and may stop after MAX_ITERATIONS
# (best-effort). It does not guarantee that every thread will be resolved.
#
# Auto-resolve verification requires explicit operator argv overrides only:
#   argv3 = EVIDENCE_TEST_PATH_OVERRIDE
#   argv4 = LINKED_COMMIT_SHA_OVERRIDE
# Bot comment bodies are never parsed for SHAs or test paths (untrusted input).
# ============================================================================
...
EVIDENCE_TEST_PATH_OVERRIDE=${3:-""}
LINKED_COMMIT_SHA_OVERRIDE=${4:-""}
...
resolve_comments_with_solutions() {
	...
	while IFS=$'\t' read -r thread_id comment_id author encoded_body path; do
		...
		if ! is_still_valid_thread "$body" "$path"; then
			pending_count=$((pending_count + 1))
			print_info "Pending thread (validity undecided or stale): $thread_id | ${path:-unknown-path}"
			continue
		fi

		if ! has_verified_targeted_fix "$body" "$path"; then
			pending_count=$((pending_count + 1))
...
is_still_valid_thread() {
	local comment_body=$1
	local file_path=$2

	if [ -z "$comment_body" ] || [ -z "$file_path" ]; then
		return 1
	fi
...
has_verified_targeted_fix() {
	local _comment_body=$1
	local file_path=$2

	# Verification is intentionally strict and does NOT trust bot review text:
	# - Operator must supply both LINKED_COMMIT_SHA_OVERRIDE (argv 4) and
	#   EVIDENCE_TEST_PATH_OVERRIDE (argv 3); untrusted comment bodies are never
	#   parsed for hex SHAs or tests/ paths.
	# - The linked commit must touch both the review file and the evidence test.
	# - The linked commit must be an ancestor of the PR head ref (rejects local-only
	#   or foreign-branch SHAs that never landed on the PR branch).
	# - No PR-diff fallback: co-touching files in a large PR is not proof of
	#   remediation and could auto-resolve security findings without a fix.

	if [ -z "$LINKED_COMMIT_SHA_OVERRIDE" ] || [ -z "$EVIDENCE_TEST_PATH_OVERRIDE" ]; then
		return 1
	fi

	local linked_sha="$LINKED_COMMIT_SHA_OVERRIDE"
	local evidence_test_path="$EVIDENCE_TEST_PATH_OVERRIDE"

	if [ -z "$file_path" ]; then
		return 1
	fi

	if ! linked_sha=$(git rev-parse --verify "${linked_sha}^{commit}" 2>/dev/null); then
		return 1
	fi

	local pr_head_ref
	pr_head_ref=$(gh pr view "$PR_NUMBER" --repo "$REPO" --json headRefOid --jq '.headRefOid' 2>/dev/null) || return 1
	if [ -z "$pr_head_ref" ]; then
		return 1
	fi
	if ! git merge-base --is-ancestor "$linked_sha" "$pr_head_ref" 2>/dev/null; then
		return 1
	fi

	if ! git ls-files --error-unmatch "$evidence_test_path" >/dev/null 2>&1; then
		return 1
	fi

	if ! git show --name-only --pretty='' "$linked_sha" | grep -Fxq "$file_path"; then
		return 1
	fi

	if ! git show --name-only --pretty='' "$linked_sha" | grep -Fxq "$evidence_test_path"; then
		return 1
	fi
...
post_solution_reply_to_comment() {
	...
	local solution_marker="<!-- resolve-pr-comments-solution:${comment_id} -->"
...
Verification details are tracked in CI and local run artifacts for this PR."
	...
	# Idempotency: if this thread already has our deterministic reply marker,
	# treat it as success. This prevents duplicate replies when reply succeeds
	# but thread resolution fails in the same iteration.
	if gh api "repos/${REPO}/pulls/${PR_NUMBER}/comments" --paginate --jq ".[] | select(.in_reply_to_id == ${comment_id}) | .body" 2>/dev/null |
		grep -qF "$solution_marker"; then
		return 0
	fi
...
post_final_summary() {
	...
	local checks_ok=0
	local unresolved_source="verified"

	if [ "${LAST_FETCH_OK:-0}" -eq 1 ]; then
		unresolved=$(count_unresolved)
		LAST_VERIFIED_UNRESOLVED=$unresolved
	else
		unresolved_source="last_verified"
		if [ "${LAST_VERIFIED_UNRESOLVED:--1}" -ge 0 ] 2>/dev/null; then
			unresolved=$LAST_VERIFIED_UNRESOLVED
		else
			unresolved=0
	...
	if [ "${LAST_FETCH_OK:-0}" -eq 1 ] && [ "$unresolved" -eq 0 ]; then
		resolution_status="ALL_ACTIONABLE_THREADS_RESOLVED"
	elif [ "${LAST_FETCH_OK:-0}" -ne 1 ]; then
		resolution_status="UNKNOWN_PENDING_REFRESH"
	fi
...
- Unresolved count source: **$unresolved_source**
...
main() {
	...
	echo -e "${YELLOW}   This loop processes actionable bot threads and may stop after MAX_ITERATIONS (best-effort)${NC}"
	echo ""
